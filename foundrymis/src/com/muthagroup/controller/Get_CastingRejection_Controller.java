...
public class Get_CastingRejection_Controller extends HttpServlet {
		...
		try {
			...
			if(company!="" && month !="" && year!=""){		
				if(company.equalsIgnoreCase("101")){ 
					db="ENGERP";
					response.sendRedirect("Monthly_CastRejection.jsp?comp="+company+"&month="+month+"&monthname="+monthName+"&db="+db);
				}
			...
			//	Example	:	exec "ENGFMSHOP"."dbo"."Sel_RptMonthlyDailyCastingProd";1 '201411', 'ENGERP'		
		} catch (Exception e) {
			e.printStackTrace();
			e.getMessage();
		}
