...
<script type="text/javascript">
	function getExcel_Report(comp,month,db,monthName) {
		...
		xmlhttp.open("POST", "IHRReport_xls.jsp?comp=" +comp+ "&month="+month+"&db="+db+"&monthName="+monthName, true);
		xmlhttp.send();
	}; 
</script>
</head>
...
// System.out.println("Company111 = " + comp + " month = " + month + " db = " + db + " month name = " + monthName);
%>
<div>
<strong style="color: blue;font-family: Arial;font-size: 14px;">MUTHA FOUNDERS PVT.LTD.&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</strong>
<strong style="color: #1B5869;font-family: Arial;font-size: 14px;">Day wise IHR For The Month <%=monthName %> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</strong><br/>
<strong style="font-size: 13px;"><a href="HomePage.jsp" style="text-decoration: none;">&lArr;BACK</a></strong>
...
%> 
			<table id="t1" border="1" cellpadding=2 style="border: 1px solid #000;"> 
				<tr align="center">
					...
					<th class="th">IHR %</th> 
					<th class="th">Rej Cost</th>
					<th class="th">Rej Rs/1000</th>
					<th class="th">Avg REJ</th>
				...
				<tr onmouseover="this.style.background='#EEEEEE';" onmouseout="this.style.background='';" >
					...
					<td class="td1" align="right"><%=rs122.getString("IHR")  %> </td>
					
					<td class="td1" align="right"><%=String.valueOf(Math.abs(Double.parseDouble(rs122.getString("REJ_COST"))))  %> </td>
					<td class="td1" align="right"><%=String.valueOf(Math.abs(Double.parseDouble(rs122.getString("REJ_RS"))))  %> </td>
					<td class="td1" align="right"><%=rs122.getString("AVG_REJ")  %> </td> 
				...
				callsp1.setString(3, db);
					...
					while (ressp1.next()) {
						...
						//	dayUpdate.add(Integer.parseInt(ressp1.getString("Dates"))-1, ressp1.getString("REJ_QTY")); 
							for(int j=0;j<(dates.size());j++){
				  ...
				  callsp.setString(3, db);
					...
					while (ressp.next()) {
						if(ressp.getString("MAT_NAME").equalsIgnoreCase(rs122.getString("MAT_NAME"))){
							//dayUpdate1.add(Integer.parseInt(ressp.getString("Dates")), ressp.getString("REJ_QTY"));							
							for(int j=0;j<(dates.size());j++){
...
<table border="1" style="border: 1px solid #000; width: 40%;height: 100px;padding-left: 20px;"> 
<tr>
<th class="th" colspan="2"><strong>SCRAP COSTS :</strong> </th>
</tr> 
<%
CallableStatement csrap = con.prepareCall("{call Sel_RptDisplayScrapLastRate(?,?,?)}");
csrap.setString(1, comp);
csrap.setString(2, month);
csrap.setString(3, db);
ResultSet rsscrap = csrap.executeQuery();
 while(rsscrap.next()){	 
	 if(!rsscrap.getString("NAME").equalsIgnoreCase("SCRAP A C")){
%>
<tr onmouseover="this.style.background='#EEEEEE';" onmouseout="this.style.background='';" >
<td style="background-color: white;" align="left"><%=rsscrap.getString("NAME") %></td>
<td style="background-color: white;" align="right"><%=rsscrap.getString("SCRP_COST")  %></td>
</tr>
<% 
	 }
 }
