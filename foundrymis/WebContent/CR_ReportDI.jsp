...
%>
	<div>
		<strong style="color: blue; font-size: 14px; font-family: Arial;">DHANASHREE INDUSTRIES&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
			&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</strong>
		<strong style="color: #1B5869; font-size: 14px; font-family: Arial;">Day wise Customer Return For The Month <%=monthName %>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		</strong><br /><strong style="font-size: 14px; font-family: Arial;"><a href="HomePage.jsp" style="text-decoration: none;">&lArr;BACK</a></strong>
	...
	CallableStatement cs = con.prepareCall("{call Sel_RptDaywiseSalesReturn(?,?,?)}");
	cs.setString(1, comp);
...
%>
			<table id="t1" border="1" cellpadding=2
				...
				<tr align="center">
					...
					<th class="th">TOTAL Sale</th>
					<th class="th">Total REJ</th>
					<%
				...
				CallableStatement cs11 = con.prepareCall("{call Sel_RptDaywiseSalesReturn(?,?,?)}");
				...
				<tr onmouseover="this.style.background='#EEEEEE';"
					...
					<td class="td1" align="left"><%=rs122.getString("GRADE_NAME") %>
					</td>
					<td class="td1" align="right"><%=rs122.getString("SCHEDULE_QTY") %>
					</td>
					<td class="td1" align="right"><%=rs122.getString("CR_PER")  %>
					</td>
					<td class="td1" align="right"><%=rs122.getString("CR_PERCENT")  %>
					</td>
					<td class="td1" align="right"><%=rs122.getString("TOT_SALEQTY")  %>
					</td>

					<% 
				CallableStatement callsp1 = con.prepareCall("{call Sel_RptDaywiseSalesReturn(?,?,?)}");
				...
				callsp1.setString(3, db);
					...
					while (ressp1.next()) {
						...
						//	dayUpdate.add(Integer.parseInt(ressp1.getString("Dates"))-1, ressp1.getString("TOT_REJ_QTY")); 
							for(int j=0;j<(dates.size());j++){
								if(Integer.parseInt(ressp1.getString("Dates"))==Integer.parseInt(dates.get(j).toString())){
									dayUpdate.set(j, ressp1.getString("TOT_REJ_QTY"));
								}						 
							}							
						}
					}
					...
					%>
					<td class="td1" align="right"><%=String.valueOf(sum).replaceAll("\\.0*$", "") %>
					</td>
					<%
					sum = 0;				
					for(int prd=0;prd<(dates.size());prd++){
					%>
					<td nowrap style="font-family: Arial; font-size: 10px;"
						align="right">&nbsp;<%=dayUpdate.get(prd).toString().replaceAll("\\.0*$", "") %></td>
					<%	
					} 	
				...
				<tr onmouseover="this.style.background='#EEEEEE';"
					...
					<% 
				  CallableStatement callsp = con.prepareCall("{call Sel_RptDaywiseSalesReturn(?,?,?)}");
				  ...
				  callsp.setString(3, db);
					...
					while (ressp.next()) {
						if(ressp.getString("MAT_NAME").equalsIgnoreCase(rs122.getString("MAT_NAME"))){
							//dayUpdate1.add(Integer.parseInt(ressp.getString("Dates")), ressp.getString("TOT_REJ_QTY"));							
							for(int j=0;j<(dates.size());j++){
								if(Integer.parseInt(ressp.getString("Dates"))==Integer.parseInt(dates.get(j).toString())){
									dayUpdate1.set(j, ressp.getString("TOT_REJ_QTY"));
								}						 
							}
						}
					} 
					...
					for(int prd1=0;prd1<(dates.size());prd1++){
						%>
					<td nowrap style="font-family: Arial; font-size: 10px;"
						align="right">&nbsp;<%=dayUpdate1.get(prd1).toString().replaceAll("\\.0*$", "") %></td>
					<%	
						}
			  ...
			  ct="";
				...
				<tr onmouseover="this.style.background='#EEEEEE';"
					...
					%>
					<td nowrap style="font-family: Arial; font-size: 10px;"
						align="right">&nbsp;<%=DayWIseSum.get(index).toString().replaceAll("\\.0*$", "") %></td>
					<%
						}
					%>
				</tr>
