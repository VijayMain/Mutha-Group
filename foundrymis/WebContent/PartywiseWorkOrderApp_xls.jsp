...
try {   //  xmlhttp.open("POST", "PartywisePorder_xls.jsp?comp=" + comp +"&sup="+sup+"&from="+from+"&to="+to, true);
	    ...
	    String tick_flag =request.getParameter("approved");
	    String flag_close =request.getParameter("closed");
	    int row=0,col=1; 

	    System.out.println("print = " + tick_flag + "  =  " + flag_close);
  ...
  // Add the created Cells to the sheet
	...
	if((tick_flag.equalsIgnoreCase("true") && flag_close.equalsIgnoreCase("true")) || (tick_flag.equalsIgnoreCase("false") && flag_close.equalsIgnoreCase("false"))){
	...
	if(tick_flag.equalsIgnoreCase("true") && flag_close.equalsIgnoreCase("false")){
	...
	if(tick_flag.equalsIgnoreCase("false") && flag_close.equalsIgnoreCase("true")){
		while(rs_close.next()){
			Label po_nolbl = new Label(row, col, rs_close.getString("TRNNO").substring(3, 7) + " - " + rs_close.getString("PO_NO") ,cellRIghtformat); 
			row++;
			Number amdatelbl = new Number(row, col, Double.parseDouble(rs_close.getString("AMEND_NO")),cellRIghtformat); 
...
%>
<span id="exportId"> 
<button id="filebutton" onclick="getExcel_Report('<%=comp%>','<%=from%>','<%=to%>','<%=tick_flag %>','<%=flag_close %>')" style="cursor: pointer;" disabled="disabled">Generate Excel</button>
