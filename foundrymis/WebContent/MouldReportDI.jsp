...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</strong>
<strong style="color: #1B5869;font-family: Arial;font-size: 14px;">Day Wise Mould Receipt of&nbsp;<%=monthName %>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</strong><br/>
...
<% 
	...
	CallableStatement cs = con.prepareCall("{call Sel_RptDaywiseMouldProd(?,?,?)}");
	cs.setString(1, comp);
	cs.setString(2, month);
	cs.setString(3, db);
	ResultSet rs1 = cs.executeQuery();
	 while(rs1.next()){
		 dates.add(rs1.getString("DATS"));
	 }
...
%> 
			<table id="t1" border="1" cellpadding=2 style="border: 1px solid #000;"> 
				<tr align="center">
					...
					<th class="th">Plan Qty</th>
					<th class="th">Prod Qty</th>
				...
				CallableStatement cs11 = con.prepareCall("{call Sel_RptDaywiseMouldProd(?,?,?)}");
				...
				<tr onmouseover="this.style.background='#EEEEEE';" onmouseout="this.style.background='';" >
					...
					<td class="td1" align="right"><%=rs122.getString("PRODQTY")  %> </td>
					<td class="td1" align="right"><%=rs122.getString("AVG_QTY")  %> </td>
					
				<% 
				CallableStatement callsp1 = con.prepareCall("{call Sel_RptDaywiseMouldProd(?,?,?)}");
				...
				callsp1.setString(3, db);
					...
					while (ressp1.next()) {
						...
						//	dayUpdate.add(Integer.parseInt(ressp1.getString("Dates"))-1, ressp1.getString("PARDAY_QTY")); 
							for(int j=0;j<(dates.size());j++){
								if(Integer.parseInt(ressp1.getString("DATS"))==Integer.parseInt(dates.get(j).toString())){
				...
				<tr onmouseover="this.style.background='#EEEEEE';" onmouseout="this.style.background='';" >
				  ...
				  <% 
				  CallableStatement callsp = con.prepareCall("{call Sel_RptDaywiseMouldProd(?,?,?)}");
				  ...
				  callsp.setString(3, db);
					...
					while (ressp.next()) {
						if(ressp.getString("MAT_NAME").equalsIgnoreCase(rs122.getString("MAT_NAME"))){
							...
							for(int j=0;j<(dates.size());j++){
								if(Integer.parseInt(ressp.getString("DATS"))==Integer.parseInt(dates.get(j).toString())){
