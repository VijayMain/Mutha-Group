...
<title>Daily Prod Founders</title>
...
<script type="text/javascript">
	function getExcel_Report(comp,month,db,monthName) {
		...
		xmlhttp.open("POST", "DailyProduction_xls.jsp?comp=" +comp+ "&month="+month+"&db="+db+"&monthName="+monthName, true);
		xmlhttp.send();
	}; 
</script>
</head>
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</strong>
<strong style="color: #1B5869; font-family: Arial;font-size: 14px;">Day wise Production For The Month  <%=monthName %> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</strong><br/>
<strong style="font-family: Arial;font-size: 14px;"><a href="HomePage.jsp" style="text-decoration: none;">&lArr;BACK</a></strong>
...
//***********************************************************************************************************
 ...
 %>	
			<table id="t1" border="1" cellpadding=2 style="border: 1px solid #000; font-family: Arial;"> 
				<tr align="center">
					...
					<th class="th">Prod Plan Per Day</th>
					<th class="th">Actual Per Day</th>
				...
				CallableStatement cs11 = con.prepareCall("{call Sel_RptDaywiseProduction(?,?,?)}");
				...
				<tr onmouseover="this.style.background='#EEEEEE';" onmouseout="this.style.background='';" >
					...
					<%--<td class="td1" align="right"><%=rs122.getString("GRADE") %> </td> --%>
					<td class="td1" align="left"><%=rs122.getString("GRADE_NAME") %> </td>
					<td class="td1" align="right"><%=rs122.getString("PLAN_QTY") %> </td>
					<td class="td1" align="right"><%=rs122.getString("PARDAY_QTY")  %> </td>
					<td class="td1" align="right"><%=rs122.getString("AVG_QTY")  %> </td>
					
				<% 
				CallableStatement callsp1 = con.prepareCall("{call Sel_RptDaywiseProduction(?,?,?)}");
				...
				callsp1.setString(3, db);
						...
						//	dayUpdate.add(Integer.parseInt(ressp1.getString("Dates"))-1, ressp1.getString("PROD_QTY")); 
							for(int j=0;j<(dates.size());j++){
								if(Integer.parseInt(ressp1.getString("Dates"))==Integer.parseInt(dates.get(j).toString())){
									dayUpdate.set(j, ressp1.getString("PROD_QTY"));
								}						 
							}							
						}
					}
					...
					%>
					<td nowrap style="font-size: 10px;" align="right">&nbsp;<%=dayUpdate.get(prd).toString().replaceAll("\\.0*$", "") %></td>
					<%	
					} 	
				...
				<tr onmouseover="this.style.background='#EEEEEE';" onmouseout="this.style.background='';" >
				  ...
				  <% 
				  CallableStatement callsp = con.prepareCall("{call Sel_RptDaywiseProduction(?,?,?)}");
				  ...
				  callsp.setString(3, db);
						...
						if(ressp.getString("MAT_NAME").equalsIgnoreCase(rs122.getString("MAT_NAME"))){
							...
							for(int j=0;j<(dates.size());j++){
								if(Integer.parseInt(ressp.getString("Dates"))==Integer.parseInt(dates.get(j).toString())){
									dayUpdate1.set(j, ressp.getString("PROD_QTY"));
								}						 
							}
						}
					}
					...
					for(int prd1=0;prd1<(dates.size());prd1++){
						%>
						<td nowrap style="font-size: 10px;" align="right">&nbsp;<%=dayUpdate1.get(prd1).toString().replaceAll("\\.0*$", "") %></td>
						<%	
						}
			  ...
			  ct="";
				%>
				 <tr onmouseover="this.style.background='#EEEEEE';" onmouseout="this.style.background='';" >
					...
					for (int index = 0; index < dates.size(); index++) {
					%>
						<td nowrap style="font-size: 10px;" style="font-size: 10px;" align="right">&nbsp;<%=DayWIseSum.get(index).toString().replaceAll("\\.0*$", "") %></td>
					 <%
						}
					%>				   					 
				</tr> 
