...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</strong>
<strong style="color: #1B5869;font-family: Arial;font-size: 14px;">Monthly Receipt of&nbsp;<%=monthName%> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</strong><br/>
<br/>
...
<% 
	...
	CallableStatement cs = con.prepareCall("{call Sel_RptMonthlyRcpt(?,?)}");
	cs.setString(1, comp);
...
%> 
			<table id="t1" border="1" cellpadding=2 style="border: 1px solid #000;"> 
				...
				CallableStatement cs11 = con.prepareCall("{call Sel_RptMonthlyRcpt(?,?)}");
				...
				<% 
				CallableStatement callsp1 = con.prepareCall("{call Sel_RptMonthlyRcpt(?,?)}");
				...
				callsp1.setString(2, month); 
					...
					while (ressp1.next()) {
						...
						//	dayUpdate.add(Integer.parseInt(ressp1.getString("DAYS"))-1, ressp1.getString("QTY")); 
							for(int j=0;j<(dates.size());j++){
								if(ressp1.getString("DAYS").equalsIgnoreCase(dates.get(j).toString())){
									dayUpdate.add(j, ressp1.getString("DC_QTY"));
								}						 
							}							
						}
					}
				...
				<tr onmouseover="this.style.background='#EEEEEE';" onmouseout="this.style.background='';" >
				  ...
				  <% 
				  CallableStatement callsp = con.prepareCall("{call Sel_RptMonthlyRcpt(?,?)}");
				  ...
				  callsp.setString(2, month); 
					...
					while (ressp.next()) {
						if(ressp.getString("NAME").equalsIgnoreCase(rs122.getString("NAME"))){
							//dayUpdate1.add(Integer.parseInt(ressp.getString("DAYS")), ressp.getString("QTY"));							
							for(int j=0;j<(dates.size());j++){
								if(ressp.getString("DAYS").equalsIgnoreCase(dates.get(j).toString())){
									dayUpdate1.add(j, ressp.getString("DC_QTY"));
								}						 
							}
						}
					}
...
<script type="text/javascript">
		...
		var myRow = freezeRow;
		var myCol = freezeCol;
		var speed = 50; //timeout speed for freez column
		var myTable;
		var noRows;
		var myCells, ID;
