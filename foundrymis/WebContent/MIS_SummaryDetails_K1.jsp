...
<script language="javascript">
	function button1(val) {
		var val1 = val; 
		document.getElementById("gradeName").value = val1;
		edit.submit();
	}
</script>
<script type="text/javascript">
	function getExcel_Report(comp,OnDateMIS,db) {
		...
		xmlhttp.open("POST", "MIS_Summaryxls_K1.jsp?comp=" +comp+ "&OnDateMIS="+OnDateMIS+"&db="+db, true);
		xmlhttp.send();
	}; 
</script>
<style type="text/css">
			...
			<table border="1" class="tftable"> 
				...
				while(rs1.next()){
			 	...
			 	<tr>	
					...
					<td scope="col" align="right"><a href="MIS_SummaryDetails_GradeK1.jsp?comp=<%=comp %>&ondate=<%=OnDateMIS %>&db=<%=db %>&grade=Cast Iron" style="text-decoration: none;"><%=cast.get(i).toString() %></a> </td>
					<td scope="col" align="right"><a href="MIS_SummaryDetails_GradeK1.jsp?comp=<%=comp %>&ondate=<%=OnDateMIS %>&db=<%=db %>&grade=SG Iron" style="text-decoration: none;"><%=sg.get(i).toString() %></a> </td>
					<%
