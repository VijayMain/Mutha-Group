...
<title>Machining Rejection</title>
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</strong>
<strong style="color: #1B5869;font-family: Arial;font-size: 14px;">Monthly Machining Rejection of&nbsp;<%=monthName%> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</strong><br/>
...
<% 
	...
	CallableStatement cs = con.prepareCall("{call Sel_RptMonthlyDailyMachiningProd(?,?)}");
...
%> 
			<table id="t1" border="1" cellpadding=2 style="border: 1px solid #000;"> 
				<tr align="center">    					 	    			
					...
					<th class="th" width="40%">Item Name</th> 
					<th class="th">Per Piece Rate</th>
					<th class="th">Avg</th>    
					<th class="th">MD PPM</th>    
				...
				CallableStatement cs11 = con.prepareCall("{call Sel_RptMonthlyDailyMachiningProd(?,?)}");
				...
				<tr onmouseover="this.style.background='#EEEEEE';" onmouseout="this.style.background='';">
					...
					<td class="td1" align="right"><%=twoDForm.format(Double.parseDouble(rs122.getString("Rs_Pc"))) %> </td>
					<% 
					avg = Double.parseDouble(rs122.getString("MR_QTY"))/dates.size();
					ppm = ((Double.parseDouble(rs122.getString("GRN_QTY")) + Double.parseDouble(rs122.getString("MR_QTY")))*100000)/Double.parseDouble(rs122.getString("GRN_QTY")); 
					%>
					<td class="td1" align="right"><%=zeroDForm.format(avg) %> </td> 
					<td class="td1" align="right"><%=zeroDForm.format(ppm) %> </td>					
				<% 
				CallableStatement callsp1 = con.prepareCall("{call Sel_RptMonthlyDailyMachiningProd(?,?)}");
				...
				callsp1.setString(2, db); 
					...
					while (ressp1.next()) {
						...
						//	dayUpdate.add(Integer.parseInt(ressp1.getString("MONTH"))-1, ressp1.getString("MR_QTY")); 
							for(int j=0;j<(dates.size());j++){
								if(ressp1.getString("MONTH").equalsIgnoreCase(dates.get(j).toString())){
									dayUpdate.set(j, ressp1.getString("MR_QTY"));
								}						 
							}							
						}
					}
				...
				<tr onmouseover="this.style.background='#EEEEEE';" onmouseout="this.style.background='';" >
					...
					<td class="td1" align="right"><%=zeroDForm.format(ppm) %></td>
				  <% 
				  CallableStatement callsp = con.prepareCall("{call Sel_RptMonthlyDailyMachiningProd(?,?)}");
				  ...
				  callsp.setString(2, db); 
					...
					while (ressp.next()) {
						if(ressp.getString("NAME").equalsIgnoreCase(rs122.getString("NAME"))){
							//dayUpdate1.add(Integer.parseInt(ressp.getString("MONTH")), ressp.getString("MR_QTY"));							
							for(int j=0;j<(dates.size());j++){
								if(ressp.getString("MONTH").equalsIgnoreCase(dates.get(j).toString())){
									dayUpdate1.set(j, ressp.getString("MR_QTY"));
								}						 
							}
						}
					}
			  ...
			  ct="";
				%>
				 <tr style="font-family: Arial;font-size: 14px;">
					...
					for (int index = 0; index < dates.size(); index++) {
					 %>
						<td nowrap style="font-family: Arial;font-size: 10px;" align="right">&nbsp;<%=twoDForm.format(Double.parseDouble(DayWIseSum.get(index).toString())) %></td>
					 <%
						}
					 %>				   					 
				</tr> 
