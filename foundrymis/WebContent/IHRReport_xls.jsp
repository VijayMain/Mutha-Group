<%@page import="java.util.concurrent.ForkJoinPool.ManagedBlocker"%>
...
try {
	...
	// System.out.println("Company = " + comp + " month = " + month + " db = " + db + " month name = " + monthName);
	...
	File exlFile = new File("C:/reportxls/IHRReport"+val+".xls");
    ...
    writableSheet.mergeCells(0, 0,8, 0);
    Label labelHeading = new Label(0, 0, CompanyName + " Day wise In House Rejection For The Month " + monthName , cellFormat);
	...
	double finalsum=0; 
	CallableStatement cs = con.prepareCall("{call Sel_RptDaywiseFoundryRej(?,?,?)}");
	cs.setString(1, comp);
...
//***********************************************************************************************************
    ...
    Label label4 = new Label(4, 1, "IHR %",cellFormat);
    Label label5 = new Label(5, 1, "Rej Cost",cellFormat);
    Label label6 = new Label(6, 1, "Rej Rs/1000",cellFormat);
    Label label7 = new Label(7, 1, "Avg REJ",cellFormat);
    Label label8 = new Label(8, 1, "Total Qty",cellFormat);
    
	for (int index = 0; index < dates.size(); index++) {
		Label label_AddMe = new Label(index+9, 1, dates.get(index).toString(),cellFormat);
	...
	int l=0,r=2;
	CallableStatement cs11 = con.prepareCall("{call Sel_RptDaywiseFoundryRej(?,?,?)}");
	...
	while (rs122.next()) {
		if(ct==""){ 
			  ...
			  Label la0 = new Label(l, r, String.valueOf(srno) ,cellFormat);
			   ...
			   l++; 
			   Label la4 = new Label(l, r, rs122.getString("IHR"),cellRIghtformat);
			   writableSheet.addCell(la4);
			   l++; 
			   Label la5 = new Label(l, r, String.valueOf(Math.abs(Double.parseDouble(rs122.getString("REJ_COST")))),cellRIghtformat);
			   writableSheet.addCell(la5);
			   l++;
			   Label la26 = new Label(l, r, String.valueOf(Math.abs(Double.parseDouble(rs122.getString("REJ_RS")))),cellRIghtformat);
			   writableSheet.addCell(la26);
			   l++;
			   Label la27 = new Label(l, r, rs122.getString("AVG_REJ"),cellRIghtformat);
			   writableSheet.addCell(la27);
			   l++;
	CallableStatement callsp1 = con.prepareCall("{call Sel_RptDaywiseFoundryRej(?,?,?)}");
			...
			//	dayUpdate.add(Integer.parseInt(ressp1.getString("Dates"))-1, ressp1.getString("PROD_QTY")); 
				for(int j=0;j<(dates.size());j++){
					if(Integer.parseInt(ressp1.getString("Dates"))==Integer.parseInt(dates.get(j).toString())){
						dayUpdate.set(j, ressp1.getString("REJ_QTY"));
					}	 
				}
			}
		}
	...
	// 888888888888888888888888888888888888888888888888888888888888888888888888888888888888888888888888888888888888888888888	
			...
			writableSheet.addCell(la5);
			l++;
			Label la25 = new Label(l, r, String.valueOf(Math.abs(Double.parseDouble(rs122.getString("REJ_RS")))),cellRIghtformat);
			writableSheet.addCell(la25);
			l++;
			Label la52 = new Label(l, r, rs122.getString("AVG_REJ"),cellRIghtformat);
			writableSheet.addCell(la52);
			l++;
	  CallableStatement callsp = con.prepareCall("{call Sel_RptDaywiseFoundryRej(?,?,?)}");
			...
			if(ressp.getString("MAT_NAME").equalsIgnoreCase(rs122.getString("MAT_NAME"))){
				...
				for(int j=0;j<(dates.size());j++){
					if(Integer.parseInt(ressp.getString("Dates"))==Integer.parseInt(dates.get(j).toString())){
						dayUpdate1.set(j, ressp.getString("REJ_QTY"));
					}						 
				}
			}
		}
 ...
 ct="";
	...
	l++; 
	Label la161 = new Label(l, r,"",cellRIghtformat);
	writableSheet.addCell(la161);
	l++; 
	Label la162 = new Label(l, r,"",cellRIghtformat);
	writableSheet.addCell(la162);
	l++; 
    ...
    String filePath = "C:/reportxls/IHRReport"+val+".xls";
%>
<span id="exportId">
