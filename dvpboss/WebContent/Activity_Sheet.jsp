...
function AddTrial(str11) {
	window.location.href = "Add_Trials.jsp?hid="+str11;
}

function addApproval(str1) {
	
	//var btn = document.getElementById("trial_Btn");
	//btn.disabled = false;
	var e1 = document.getElementById("approval_Btn");
	e1.setAttribute('disabled', 'disabled');
	
	document.getElementById("approvalImages").style.visibility = "visible";
	...
	xmlhttp1.onreadystatechange = function() {
		if (xmlhttp1.readyState == 4 && xmlhttp1.status == 200) {
			document.getElementById("getApr").innerHTML = xmlhttp1.responseText;
		}
	};
	xmlhttp1.open("POST", "Send_Approval.jsp?sheetId=" + str1, true);
	xmlhttp1.send();
}; 
</script>
...
- -->
	<%
		...
		System.out.println("basic id sheet =" + basic_id);
	%>
		...
		<form action="Approval_Controller" class="register" method="post"
			...
			<table border="1" class="tftable">
				...
				<tr style="background-color: #acc8cc" align="center">
					...
					<td scope="col"><strong>3d Model</strong></td>
			...
			<table width="35%">
				...
				<tr onMouseOver="ChangeColor(this, true);"
					...
					<td width="22%"><input type="button" name="approval"
						id="approval_Btn" value="Send it for Approval"
						onclick="addApproval('<%=basic_id%>')" /></td>
					...
					<%
						PreparedStatement ps_trial = con
									.prepareStatement("select * from dev_approval_tbl where Basic_Id="
											+ basic_id + " and Approval_id=1");
							ResultSet rs_trial = ps_trial.executeQuery();

							rs_trial.last();
							int tr = rs_trial.getRow();
							rs_trial.beforeFirst();
							if (tr > 0) {
					%>
					<td width="52%"><input type="button" name="Add_Trial"
						id="trial_Btn" value="Add Trials"
						onclick="AddTrial('<%=basic_id%>')" /></td>
				...
				<tr>
					<td colspan="3"><span id="getApr"> <img
							src="images/dots64.gif" id="approvalImages" width="64"
							height="21" style="visibility: hidden;" />
					</span></td>
				</tr>
