	...
	<select id="part_name" name="part_name" onchange="getpartno(this.value)" style="cursor: pointer; max-width: 11em;">
		...
		<%
			...
			try {
				...
				ps_getCust_Id=con.prepareStatement("select cust_id from customer_tbl where cust_name='"+str+"'");
				
				ResultSet rs_getCust_Id=ps_getCust_Id.executeQuery();
				while(rs_getCust_Id.next())
				{
					ps_getPart_Ids = con
							.prepareStatement("select PartName_Id from cs_cust_partname_rel_tbl where Cust_Id="
									+ rs_getCust_Id.getInt("cust_id"));
				...
				while (it.hasNext()) 
				{
					...
					{
						partName=rs_partName.getString("Part_Name");
		%>
