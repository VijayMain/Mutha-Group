...
import com.muthagroup.bo.Edit_NewSheet_bo;
...
public class Edit_NewSheet_Controller extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			int basic_id = 0;
			Edit_NewSheet_vo bean = new Edit_NewSheet_vo();
			Edit_NewSheet_dao dao = new Edit_NewSheet_dao();
			Edit_NewSheet_bo bo = new Edit_NewSheet_bo();
					...
					while (it.hasNext()) {
						...
						if (fileItemTemp.isFormField()) {
							...
							if (fieldName.equalsIgnoreCase("sheet_no")) {
								...
								System.out.println("sheet_no == " + bean.getSheet_no());
							}
							...
							if (fieldName.equalsIgnoreCase("new_customer")) {
								System.out.println("new casting vendor === "
										+ fieldValue);
								bean.setNew_customer(fieldValue);
							}
							if (fieldName.equalsIgnoreCase("new_part_name")) {
								System.out.println("new part name === "
										+ fieldValue);
								bean.setNew_part_name(fieldValue);
							}
							if (fieldName.equalsIgnoreCase("new_part_no")) {
								System.out.println("new part no === "
										+ fieldValue);
								bean.setNew_part_no(fieldValue);
							}
							if (fieldName.equalsIgnoreCase("new_grade_type")) {
								System.out.println("new grade type === "
										+ fieldValue);
								bean.setNew_grade_type(fieldValue);
							}
							if (fieldName.equalsIgnoreCase("new_plant_vendor")) {
								System.out.println("new plant vendor === "
										+ fieldValue);
								bean.setNew_plant_vendor(fieldValue);
							}
							if (fieldName.equalsIgnoreCase("new_casting_vendor")) {
								System.out.println("new casting vendor === "
										+ fieldValue);
								bean.setNew_casting_vendor(fieldValue);
							}
							if (fieldName.equalsIgnoreCase("sheet_no")) {
							...
							for (int k = 1; k <= bean.getSrNo(); k++) {
								...
								if (fieldName.equalsIgnoreCase("inputName" + k)) {
									...
									if (k == 1
											...
											&& bean.getCust_name() != null || bean.getNew_customer()!=null
											&& bean.getGrade_type() != null || bean.getNew_grade_type()!=null
											&& bean.getCV_name() != null || bean.getNew_casting_vendor() !=null || bean.getCV_name()==null || bean.getNew_casting_vendor()==null || bean.getCasting_from()==0 || bean.getCasting_from()!=0
											&& bean.getPV_name() != null || bean.getNew_plant_vendor() !=null || bean.getPV_name()==null || bean.getNew_plant_vendor()==null || bean.getPlant_name()!=0 || bean.getPlant_name()==0
											&& bean.getMarketing_apr() != null
											&& bean.getPo_number() != null
											&& bean.getPart_name() != null || bean.getNew_part_name()!=null
											...
											&& bean.getPo_received_date() != null)  
									{
										...
										basic_id = bo.updateNewSheet(request, bean);
										
										System.out.println("Basic Id k==1"+ basic_id);
									}
									...
									if (bean.getFile_Name_ext() != null) 
									{
										...
										System.out
												.println("Basic Id =  == == == "
														+ basic_id);
										flag = dao.updateAttach_File(bean, session,
												basic_id);
									}
