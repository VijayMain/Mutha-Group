...
public class ECN_ActionCustCheck_mailer extends TimerTask {
	@Override
	public void run() {
		...
		ArrayList rem = new ArrayList();
			 if (weekday[d.getDay()].equals("Wednesday") && d.getHours() == 9 && d.getMinutes() == 35) {
		/*if (d.getHours() == 17 && d.getMinutes() == 16) {*/
			try {
				...
				ArrayList countEmail = new ArrayList(); 
					...
					PreparedStatement psap = con.prepareStatement("select distinct(CRC_No),u_id from crc_tbl_action where Action_Date between '"+firstDate+"' and '"+nowDate+"'"); 
					ResultSet rsap = psap.executeQuery();
					while (rsap.next()) {
						
						ps_ap = con.prepareStatement("select * from crc_tbl_approval where Approval_Id=1 and CRC_No="+ rsap.getInt("CRC_No"));
						...
						while (rs_user.next()) {
							...
							System.out.println("Emails List = " + rs_user.getString("U_Email") + " Cr No = " + rsap.getInt("CRC_No"));
							}
								} 
					}	
			...
			sb.append("<table border='1' width='100%' style='font-family: Arial;'>"+
						...
						"<th width='8%' height='25' colspan='8'><b>Please <a href='http://192.168.0.7/ECN/'>click here</a> and Verify the Actions on following ECN(Customer Change Note)</b></th>"+
						"</tr>"+
			...
			ps_apr = con.prepareStatement("select distinct(CRC_No) from crc_tbl_action where Action_Date between '"+firstDate+"' and '"+nowDate+"'");
			...
			for(int c=0;c<approvers.size();c++){
						...
						ps_rd = con.prepareStatement("select * from crc_tbl where CRC_No="+Integer.parseInt(approvers.get(c).toString()));
			...
			sb.append("<tr style='font-size: 9px;font-family: Arial;'>"+
					"<td style='text-align: center;'>"+rs_rd.getInt("CRC_No")+"</td>"+
					"<td style='text-align: left;'>"+partname+"</td>"+
					"<td style='text-align: center;'>"+rs_rd.getDate("CRC_Date")+"</td>"+
					"<td style='text-align: center;'>"+rs_rd.getDate("Targated_Impl_Date")+"</td>"+
					"<td style='text-align: left;'>"+rs_rd.getString("Nature_Of_Change")+"</td>"+
					"<td style='text-align: left;'>"+rs_rd.getString("Reason_For_Change")+"</td>"+
					"<td style='text-align: left;'>"+rs_rd.getString("Change_For")+"</td>"+
