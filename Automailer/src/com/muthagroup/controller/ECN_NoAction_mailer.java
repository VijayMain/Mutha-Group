...
public class ECN_NoAction_mailer extends TimerTask { 
	@Override
	public void run() {
		...
		ArrayList rem = new ArrayList(); 
			if (weekday[d.getDay()].equals("Wednesday") && d.getHours() == 9 && d.getMinutes() == 32) {
		/*if (d.getHours() == 17 && d.getMinutes() == 7) {*/
			try {  
				...
				ps_apr = con.prepareStatement("select * from cr_tbl");
				...
				ps_cr = con.prepareStatement("select * from cr_tbl_approval where Approval_Id in(2,3)");
				rs_cr = ps_cr.executeQuery();
				while (rs_cr.next()) {
					if (approvers.contains(rs_cr.getInt("CR_No"))) {
						app_rem.add(rs_cr.getInt("CR_No"));
					}
				}
			...
			String subject = "No Action Registered on Approved ECN(Internal Change Requests)";
			...
			PreparedStatement psap = con.prepareStatement("select * from cr_tbl where CR_No="+Integer.parseInt(approvers.get(c).toString())); 
		...
		//	recipients[ct] = "govindmb@muthagroup.com";
			...
			sb.append("<p><b>Please <a href='http://192.168.0.7/ECN/'>click here</a> to Approve.</b></p>" +
					"<table border='1' width='100%' style='font-family: Arial;'>"+
						...
						"<th width='8%' height='25' colspan='9'><b style='color: red;'>No Action Taken on Approved ECN(Internal Change Requests)</b></th>"+
						"</tr>"+
			...
			for (int i = 0; i < approvers.size(); i++) {
				ps_pr = con.prepareStatement("select * from cr_tbl where CR_No=" + Integer.parseInt(approvers.get(i).toString()));
			...
			sb.append("<tr style='font-size: 9px;'>"+
					"<td style='text-align: center;'>"+rs_pr.getInt("CR_No")+"</td>"+
					"<td style='text-align: left;'>"+partname+"</td>"+
					"<td style='text-align: center;'>"+rs_pr.getDate("CR_Date")+"</td>"+
					"<td style='text-align: center;'>"+rs_pr.getDate("Proposed_Impl_Date")+"</td>"+
					"<td style='text-align: left;'>"+rs_pr.getString("Present_System")+"</td>"+
					"<td style='text-align: left;'>"+rs_pr.getString("Proposed_System")+"</td>"+
					"<td style='text-align: left;'>"+rs_pr.getString("Objective")+"</td>"+
