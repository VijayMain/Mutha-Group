...
public class ECN_Automailer extends TimerTask {
	@Override
	public void run() {		
		Date d = new Date();
		...
		/*if (d.getHours() == 17 && d.getMinutes() == 01) {*/
			try {
				...
				int validcount=0;
				PreparedStatement ps = con.prepareStatement("select count(*) from cr_tbl_approval where CR_Approval_Id=2"); 
			...
			PreparedStatement psap = con.prepareStatement("select * from cr_tbl_approval where CR_Approval_Id=2"); 
			...
			sb.append("<p><b>Please <a href='http://192.168.0.7/ECN/'>click here</a> to Approve.</b></p>" +
					"<table border='1' width='100%' style='font-family: Arial;'>"+
						...
						"<th width='8%' height='25' colspan='9'><b style='color: red;'>Internal Change Request Pending Approvals</b></th>"+
						"</tr>"+
			...
			ps_apr = con.prepareStatement("select CR_No from cr_tbl_approval where Approval_Id=3");
			rs_apr = ps_apr.executeQuery();
			while (rs_apr.next()) { 
				approvers.add(rs_apr.getInt("CR_No")); 
			}
			
				PreparedStatement ps_data = con.prepareStatement("select * from cr_tbl order by CR_Date");
				ResultSet rs_data = ps_data.executeQuery();
				while (rs_data.next()) { 
					
				if(!approvers.contains(rs_data.getInt("CR_No"))){
										
				ps_ecn = con.prepareStatement("select distinct(CR_No) from cr_tbl_approval where Approval_Id=2 and CR_No="+ rs_data.getInt("CR_No"));
			...
			sb.append("<tr style='font-size: 9px;font-family: Arial;'>"+
					"<td style='text-align: center;'>"+rs_data.getInt("CR_No")+"</td>"+
					"<td style='text-align: left;'>"+partname+"</td>"+
					"<td style='text-align: center;'>"+rs_data.getDate("CR_Date")+"</td>"+
					"<td style='text-align: center;'>"+rs_data.getDate("Proposed_Impl_Date")+"</td>"+
					"<td style='text-align: left;'>"+rs_data.getString("Present_System")+"</td>"+
					"<td style='text-align: left;'>"+rs_data.getString("Proposed_System")+"</td>"+
					"<td style='text-align: left;'>"+rs_data.getString("Objective")+"</td>"+
