...
function validateForm() {   
	...
	var inputoutput = document.getElementById("inputoutput");  
		...
		if (ipaddress.value=="0" || ipaddress.value==null || ipaddress.value=="" || ipaddress.value=="null" || ipaddress.value=="192.168.0.") {
			alert("Please Enter New IP Address !!!"); 
...
<div id="container">
  <div id="top">
    <h3><strong>Add IP Address</strong> </h3> 
  </div>
  <div id="menu">
			<ul>
  ...
  <div style="width:50%; height: 85%; padding-left: 10px;padding-bottom: 5px;padding-top: 5px;float: left;"> 
  	...
  	<form action="Add_NewIPAddress_Controller" method="post"  onSubmit="return validateForm();">
		<table border="0">
			<tr>
				<td>IP Address :</td>
				<td> <input type="text" name="ipaddress" id="ipaddress" size="35" style="height: 20px;" value="192.168.0."/> </td>
			</tr>   
			<tr> 
  ...
  <div style="width:40%; height:550px;float: right;padding-left: 10px;padding-right:5px; padding-bottom: 5px;padding-top: 5px;overflow: scroll;"> 
 	<table border="1" style="font-size: 12px; color: #333333; width: 50%; border-width: 1px; border-color: #729ea5; border-collapse: collapse;">
	<tr>
	<td  style="background-color: #acc8cc; border-width: 1px; padding: 8px; border-style: solid; border-color: #729ea5; text-align: center;"><b>Available IP Address</b></td> 
	</tr> 
	<%
	PreparedStatement ps_adip=con.prepareStatement("select * from it_asset_ipaddress_mst_tbl");
	ResultSet rs_adip=ps_adip.executeQuery();
	while(rs_adip.next()){  
	%>
	<tr align="center">	
	<td style="border-width: 1px; padding: 8px; border-style: solid; border-color: #729ea5; text-align: center;"><%=rs_adip.getString("ip_address") %></td>
	</tr>
	<% 
		}
	ps_adip.close();
