...
function validateForm() {
	var new_os = document.getElementById("new_os");
	var ostype = document.getElementById("ostype");  
	 
		if (new_os.value=="0" || new_os.value==null || new_os.value=="" || new_os.value=="null") {
			alert("Please Enter New Operating System !!!"); 
			document.getElementById("ADD").disabled = false;
			return false;
		}
		if (ostype.value=="0" || ostype.value==null || ostype.value=="" || ostype.value=="null") {
...
<div id="container">
  <div id="top">
    <h3><strong>Add New Operating System</strong> </h3> 
  </div>
  <div id="menu">
			<ul>
  ...
  <br/> 
  	<form action="Add_NewOS_Controller" method="post" onsubmit="return validateForm();">
		<table border="0">
			<tr>
				<td>New Operating System :</td>
				<td> <input type="text" name="new_os" id="new_os" size="35" style="height: 20px;"/> </td>
			</tr>
			<tr>
				<td>License Type :</td>
				<td> 
				<select name="ostype" id="ostype" style="height: 30px;width: 280px;">
  ...
  <div style="width:40%; height: 500px;float: right;padding-left: 10px;padding-bottom: 5px;padding-top: 5px;overflow: scroll;"> 
	...
	<tr>
	<td colspan="2" style="background-color: #acc8cc; border-width: 1px; padding: 8px; border-style: solid; border-color: #729ea5; text-align: center;"><b>Available Operating System</b></td> 
	</tr>
	<tr>
	...
	<%
	PreparedStatement ps_soft=con.prepareStatement("select * from it_asset_os_mst_tbl");
	ResultSet rs_soft=ps_soft.executeQuery();
	while(rs_soft.next()){  
	%>
	<tr align="center">	
	<td style="border-width: 1px; padding: 8px; border-style: solid; border-color: #729ea5; text-align: center;"><%=rs_soft.getString("os_name") %></td>
	<%
