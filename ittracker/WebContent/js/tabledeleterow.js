...
/*
 * insertRowToTable Insert and reorder
 */
	...
	if (hasLoaded) {
		...
		for ( var i = 0; i < tbl.tBodies[0].rows.length; i++) {
			if (tbl.tBodies[0].rows[i].myRow
					&& tbl.tBodies[0].rows[i].myRow.four.getAttribute('type') == 'radio'
					&& tbl.tBodies[0].rows[i].myRow.four.checked) {
				rowToInsertAt = i;
				break;
			}
		}
...
/*
 * addRowToTable Inserts at row 'num', or appends to the end if no arguments are
...
function addRowToTable(num) {
	if (hasLoaded) {
		...
		var elem = document.getElementById("srno");
		elem.value = iteration;
		...
		btnEl.setAttribute('class', 'button_text');
...
function deleteChecked() {
	if (hasLoaded) {
		...
		for ( var i = 0; i < tbl.tBodies[0].rows.length; i++) {
			if (tbl.tBodies[0].rows[i].myRow
					&& tbl.tBodies[0].rows[i].myRow.three.getAttribute('type') == 'checkbox'
					&& tbl.tBodies[0].rows[i].myRow.three.checked) {
	...
	if (hasLoaded) {
		if (tbl.tBodies[0].rows[startingIndex]) {
			...
			for ( var i = startingIndex; i < tbl.tBodies[0].rows.length; i++) {
				...
				// CONFIG: next line is affected by myRowObject settings
				tbl.tBodies[0].rows[i].myRow.two.name = INPUT_NAME_PREFIX
						+ count; // input text

				// CONFIG: next line is affected by myRowObject settings
				var tempVal = tbl.tBodies[0].rows[i].myRow.two.value.split(' '); // for
				// debug
				// purposes
				tbl.tBodies[0].rows[i].myRow.two.value = count + ' was'
						+ tempVal[0]; // for debug purposes

				// CONFIG: next line is affected by myRowObject settings
				tbl.tBodies[0].rows[i].myRow.four.value = count; // input
				// radio
