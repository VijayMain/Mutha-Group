...
function getAllReports(str) {
	...
	xmlhttp.onreadystatechange = function() {
		if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
			document.getElementById("getReportname").innerHTML = xmlhttp.responseText; 
		}
	}; 
	xmlhttp2.onreadystatechange = function() {
		if (xmlhttp2.readyState == 4 && xmlhttp2.status == 200) {
			document.getElementById("getParts").innerHTML = xmlhttp2.responseText; 
		}
	};
	
	xmlhttp.open("POST", "AllReports.jsp?q=" + str, true); 
	xmlhttp2.open("POST", "Get_AllCompanyReports.jsp?q=" + str, true);
	xmlhttp.send();
	xmlhttp2.send();
}; 
 
function getAllParts(str) {
	document.getElementById("waitImagep_order").style.visibility = "visible";
	var str1 = document.getElementById("comphid").value;
	var xmlhttp;
	var xmlhttp1;
	var xmlhttp2;
	if (window.XMLHttpRequest) {
	...
	xmlhttp.onreadystatechange = function() {
		if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
			document.getElementById("getSelectedParts").innerHTML = xmlhttp.responseText; 
		}
	};
	xmlhttp1.onreadystatechange = function() {
		if (xmlhttp1.readyState == 4 && xmlhttp1.status == 200) {
			document.getElementById("getItems").innerHTML = xmlhttp1.responseText;   
		}
	};
	xmlhttp2.onreadystatechange = function() {
		if (xmlhttp2.readyState == 4 && xmlhttp2.status == 200) {
	...
	xmlhttp.open("POST", "SelectedParts.jsp?q=" + str + "&s="+str1, true);
	xmlhttp1.open("POST", "SelectedItemSub.jsp?q=" + str + "&s="+str1, true);
	xmlhttp2.open("POST", "BlankItemSub.jsp?q=" + str + "&comp="+str1, true);  
	xmlhttp.send(); 
	xmlhttp1.send();
	xmlhttp2.send(); 
 };
 
 function getReset(str,str1) {
    if (confirm("Are you sure to clear all configuration ?") == true) {
    	var xmlhttp;
		if (window.XMLHttpRequest) {
			// code for IE7+, Firefox, Chrome, Opera, Safari
			xmlhttp = new XMLHttpRequest(); 
		...
		xmlhttp.onreadystatechange = function() {
			if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
				document.getElementById("reset_data").innerHTML = xmlhttp.responseText; 
			}
		};
		xmlhttp.open("POST", "ResetParts.jsp?q=" + str +"&comp="+str1, true);  
		xmlhttp.send();
    } 
	 };
 
 function getSearchedPart(str) {
	 var comp = document.getElementById("company").value;
		...
		xmlhttp.onreadystatechange = function() {
			if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
				document.getElementById("getParts").innerHTML = xmlhttp.responseText; 
			}
		};
		xmlhttp.open("POST", "SearchedParts.jsp?q=" + str +"&comp="+comp, true);  
		xmlhttp.send(); 
	 };
	
	 function getAllAC_Names(str) {
	 var comp = document.getElementById("company").value;
		var xmlhttp;  
		if (window.XMLHttpRequest) {
			// code for IE7+, Firefox, Chrome, Opera, Safari
		...
		xmlhttp.open("POST", "Get_AC_names.jsp?q=" + str +"&comp="+comp, true);  
		xmlhttp.send(); 
	 };
	 
	 function getAllcustsub(str) {
		 document.getElementById("waitImagep_order").style.visibility = "visible";
		 
		 var comp = document.getElementById("company").value;
		 var fromDatehid = document.getElementById("fromDatehid").value;
		 var toDatehid = document.getElementById("toDatehid").value;
		 var reporthid = document.getElementById("reporthid").value;
			...
			xmlhttp1.onreadystatechange = function() {
				if (xmlhttp1.readyState == 4 && xmlhttp1.status == 200) {
					document.getElementById("getSelectedParts").innerHTML = xmlhttp1.responseText; 
				}
			};
			xmlhttp.open("POST", "Get_customerall.jsp?q=" + str +"&comp="+comp + "&from="+fromDatehid+"&to="+toDatehid+"&report="+reporthid, true);
			xmlhttp1.open("POST", "Customer_allSelected.jsp?q=" + str +"&comp="+comp+ "&from="+fromDatehid+"&to="+toDatehid+"&report="+reporthid, true);  
			xmlhttp.send();
			xmlhttp1.send();
		 };
</script>
...
function validateConfigForm() {
	...
	var report = document.getElementById("report");
		...
		if (sbTwo.value=="0" || sbTwo.value==null || sbTwo.value=="" || sbTwo.value=="null") {
			alert("Please Select Names by clicking >> button !!!"); 
			document.getElementById("config_btn").disabled = false;
			document.getElementById("waitImageconfig").style.visibility = "hidden";
			return false;
		}
       ...
       <!--  <div id="header_phone_no"> 
        ...
        <div id="tooplate_menu">        	
            ...
            <ul>
                ...
                <li><a href="Configure.jsp" class="current">CONFIGURE</a></li>
...
try{
%>
<form action="Configure_Controller" method="post" id="config_form" name="config_form" onsubmit="return validateConfigForm();"> 
<h4>Configure Reports &#8658;</h4>
        <table width="100%" border="0">    
        <tr>
        <td colspan="4">
		<b>Company Name : </b>
		<select name="company" id="company" style="height: 25px;background-color: #EBECED;" onchange="getAllReports(this.value)">
		...
		<input type="hidden" name="comphid" id="comphid"/>
		<select name="report" id="report" style="height: 25px;background-color: #EBECED;" onchange="getAllParts(this.value)">
			<option value="">- - - - - - Select - - - - - -</option>
			<option value=""></option>
        </select>
        </span>
		...
		<span id="waitImagep_order" style="visibility: hidden;"><strong style="color: blue;">Please Wait while loading......</strong></span>
        			...
        			<tr>
						<td colspan="4"><b>Select Multiple : </b></td>
					</tr>
					<tr>
    					...
    					<select id="sbTwo" name="sbTwo" multiple="multiple" style="height: 200px;width: 400px;overflow: scroll;font-family: Arial;font-size: 12px;"> 
    					</select>
    					</span>
						</td>
					</tr>
					...
					<tr>
						<td colspan="4">
						<input type="submit" value="Submit" id="config_btn" style="height: 30px;width: 200px;font-weight: bold;"/>
