...
function getAllVendors(str) { 
	...
	xmlhttp2.onreadystatechange = function() {
		if (xmlhttp2.readyState == 4 && xmlhttp2.status == 200) {
			document.getElementById("getReports").innerHTML = xmlhttp2.responseText; 
		}
	};
	
	xmlhttp.open("POST", "AllReportNames.jsp?q=" + str, true); 
	xmlhttp2.open("POST", "Get_AllSelectedReports.jsp?q=" + str, true);
	xmlhttp.send(); 
	xmlhttp2.send();
}; 
...
function validateConfigForm() {
	...
	var sbTwo = document.getElementById("sbTwo");
		...
		if (sbTwo.value=="0" || sbTwo.value==null || sbTwo.value=="" || sbTwo.value=="null") {
			alert("Please Select Report Names by clicking >> button !!!"); 
			document.getElementById("config_btn").disabled = false;
			document.getElementById("waitImageconfig").style.visibility = "hidden";
			return false;
		}
...
try{
%>
<form action="Configure_Companywise_Controller" method="post" onsubmit="return validateConfigForm();"> 
<h4>Configure Company wise Reports &#8658;</h4>
		...
		<select name="company" id="company" style="height: 25px;background-color: #EBECED;" onchange="getAllVendors(this.value)">
			...
			<option value="103">MFPL</option> 
 			...
 			<option value="106">MEPL Unit III</option>
        			...
        			<tr>
						<td colspan="4"><b>Select Multiple Report Names : </b></td>
					</tr>
					<tr>
    					...
    					<span id="getReports">	
    					<select id="sbTwo" name="sbTwo" multiple="multiple" style="height: 200px;width: 400px;font-size:12px;overflow: scroll;"> 
    					</select>
    					</span>
						</td>
					</tr>
