		...
		.extend(
				...
				function() {
					...
					var f = function(b, c) {
						...
						var k = c.data || null;
						var l = c.processData === undefined ? true
								: c.processData;
						var m = c.headers || {};
						...
						if (p && window.XDomainRequest
							...
							return g(b, c);
						if (c.progress && r.upload)
							r.upload.addEventListener("progress", function(a) {
								if (a.lengthComputable)
									c.progress(Math.round((a.loaded * 95)
											/ a.total));
							}, false);
						var s = function() {
							if (r.readyState == 4 && !q) {
					...
					var g = function(b, c) {
						...
						if (c.async)
							throw new a.FilepickerException(
									"Asyncronous Cross-domain requests are not supported");
						if (e != "GET" && e != "POST") {
							h._method = e;
							e = "POST";
						}
						if (c.processData !== false)
							h = h ? d(h) : null;
						if (h && e == 'GET') {
							b += (b.indexOf('?') >= 0 ? '&' : '?') + h;
							h = null;
						...
						b += (b.indexOf('?') >= 0 ? '&' : '?')
								+ '_xdr=true&_cacheBust=' + a.util.getId();
						...
						i.onerror = function() {
							if (c.progress)
								c.progress(100);
							g(i.responseText || "CORS_error",
									this.status || 500, this);
						};
					...
					var c = function(a, c) {
						...
						while (m < a.length) {
							...
							if (isNaN(g))
								...
								l = 64;
							d = d + b.charAt(i) + b.charAt(j) + b.charAt(k)
									+ b.charAt(l);
						}
						return d;
					};
					...
					var f = function(a) {
						...
						while (c < a.length) {
							...
							if (d < 128) {
								...
								c2 = a.charCodeAt(c + 1);
								b += String.fromCharCode(((d & 31) << 6)
										| (c2 & 63));
								c += 2;
							} else {
								c2 = a.charCodeAt(c + 1);
								c3 = a.charCodeAt(c + 2);
								b += String.fromCharCode(((d & 15) << 12)
										| ((c2 & 63) << 6) | (c3 & 63));
								c += 3;
							}
						}
...
filepicker
		.extend(
				...
				function() {
					...
					var e = function() {
						return (navigator.userAgent.search("Safari") >= 0 && navigator.userAgent
								.search("Chrome") < 0);
					};
					var f = function() {
						return !!(navigator.userAgent
								.match(/(iPod|iPhone|iPad)/)
								&& navigator.userAgent.match(/AppleWebKit/) && !navigator.userAgent
								.match(/CriOS/));
					};
					var g = function() {
						var a = window.navigator.userLanguage
...
filepicker.extend("comm", function() {
	...
	var e = function(b) {
		if (b.origin != a.urls.BASE && b.origin != a.urls.DIALOG_BASE)
			return;
	...
	var g = function() {
		...
		if (window.addEventListener)
			...
			throw new a.FilepickerException("Unsupported browser");
	};
	var h = function() {
...
filepicker.extend("comm_fallback", function() {
	...
	var h = function() {
		if (window.frames[b] === undefined) {
			...
			c.id = c.name = b;
			c.src = a.urls.FP_COMM_FALLBACK + "?host_url="
	...
	var c = function(c) {
		...
		for ( var e in c) {
			...
			for ( var f in b)
				if (e == f) {
					d = true;
					if (b[f].indexOf(a.util.typeOf(c[e])) === -1)
						throw new a.FilepickerException("Conversion parameter "
								+ e + " is not the right type: " + c[e]
								+ ". Should be a " + b[f]);
				}
			if (!d)
				throw new a.FilepickerException("Conversion parameter " + e
						+ " is not a valid parameter.");
		}
	};
	var d = function(b, d, e, f, g) {
		c(d);
		if (d.crop && a.util.isArray(d.crop))
			d.crop = d.crop.join(",");
		a.ajax.post(b + '/convert', {
			...
			error : function(b, c, d) {
				if (b == "not_found")
					f(new a.errors.FPError(141));
				else if (b == "bad_params")
					f(new a.errors.FPError(142));
				else if (b == "not_authorized")
					f(new a.errors.FPError(403));
				else
					f(new a.errors.FPError(143));
			},
			progress : g
		});
	};
...
filepicker
		.extend(
				...
				function() {
					var a = this;
					var b = function() {
						return (!!window.FileReader || navigator.userAgent
								.indexOf("Safari") >= 0)
					...
					var c = function(c, d) {
						...
						if (!c)
							throw new a.FilepickerException(e);
						if (c.jquery) {
							if (c.length === 0)
								throw new a.FilepickerException(e);
							c = c[0];
						...
						if (!b()) {
							a.util.console
									.error("Your browser doesn't support drag-drop functionality");
							return false;
						}
						d = d || {};
						...
						if (!l)
							if (d.mimetype)
								...
								l = [ "*/*" ];
						if (a.util.typeOf(l) == 'string')
							l = l.split(',');
						var m = d.extensions;
						if (!m)
							if (d.extension)
								m = [ d.extension ];
						if (a.util.typeOf(m) == 'string')
							m = m.replace(/ /g, "").split(',');
						var n = {
							location : d.location,
							path : d.path,
						...
						var o = function() {
							return c
									&& (c.getAttribute("disabled") || "enabled") != "disabled";
						};
										...
										function(b) {
											...
											if (b.dataTransfer.items) {
												...
												for (f = 0; f < d.length; f++) {
													e = d[f]
															&& d[f].webkitGetAsEntry ? d[f]
															.webkitGetAsEntry()
															: undefined;
													if (e && !!e.isDirectory) {
														j("WrongType",
																"Uploading a folder is not allowed");
														return false;
													}
												}
											}
						...
						var w = function(b) {
							if (b.length > 0) {
								...
								if (d.maxFiles > 0 && b.length > d.maxFiles) {
									j("TooManyFiles", "Only " + d.maxFiles
											+ " files at a time");
									return false;
								}
								var c;
								...
								for (var g = 0; g < b.length; g++) {
									...
									if (!c) {
										j(
												"WrongType",
												f
														+ " isn't the right type of file");
										return false;
									}
									if (m) {
										c = false;
										for (var k = 0; k < m.length; k++)
											c = c || a.util.endsWith(f, m[k]);
										if (!c) {
											j(
													"WrongType",
													f
								...
								j("NoFilesFound", "No files uploaded");
							return false;
						};
						return true;
					};
...
filepicker.extend("errors", function() {
	...
	var b = function(a) {
		...
		if (filepicker.debug) {
			...
			this.toString = function() {
				return "FPError " + this.code + ": " + this.message
						+ ". For help, see " + this.moreInfo;
			};
		} else
			this.toString = function() {
				return "FPError " + this.code
	...
	var c = function(b) {
		if (filepicker.debug)
			a.util.console.error(b.toString());
	};
	return {
		FPError : b,
		handleError : c
		...
		.extend(
				...
				function() {
					...
					var b = function(b) {
						...
						if (b.mimetype && b.extension)
							throw a
									.FilepickerException("Error: Cannot pass in both mimetype and extension parameters to the export function");
						c('service', 'services');
						if (b.services)
						...
						if (b.openTo)
							b.openTo = a.services[b.openTo] || b.openTo;
						a.util.setDefault(b, 'container', 'modal');
					};
					var c = function(b, c) {
						var d = function(d) {
							if (d.type !== "filepickerUrl")
								return;
							if (d.error) {
					...
					var d = function(e, f, g, h) {
						...
						if (f.container == 'modal')
							if (a.window.shouldForce())
								f.container = 'window';
							else if ((a.browser.isIOS() || a.browser
									.isAndroid())
									&& a.browser.isSafari())
								f.mobile = true;
						a.window.open(f.container, f.mobile, a.urls
								.constructExportUrl(e, f, j), n);
				...
				function() {
					...
					var c = function(b, c, d, e, f) {
						...
						a.ajax.get(b, {
							...
							success : d,
							error : function(b, c, d) {
								if (b == "CORS_not_allowed")
									e(new a.errors.FPError(113));
								else if (b == "CORS_error")
									e(new a.errors.FPError(114));
								else if (b == "not_found")
									e(new a.errors.FPError(115));
								else if (b == "bad_params")
									e(new a.errors.FPError(400));
								else if (b == "not_authorized")
									e(new a.errors.FPError(403));
								else
					...
					var d = function(b, c, d, e, f) {
						if (!(window.File && window.FileReader
							...
							a.files.storeFile(b, {}, function(b) {
								f(50);
								a.files.readFromFPUrl(b.url, c, d, e, function(
										a) {
									f(50 + a / 2);
								});
							}, e, function(a) {
						...
						i.onprogress = function(a) {
							if (a.lengthComputable)
								f(Math.round((a.loaded / a.total) * 100));
						};
						i.onload = function(b) {
							f(100);
							if (g)
								d(a.base64.encode(b.target.result, h));
							else
								d(b.target.result);
						};
						...
						if (h || !i.readAsBinaryString)
							i.readAsText(b);
						else
							i.readAsBinaryString(b);
					};
					var e = function(b, c, d, e, f, g) {
						...
						a.ajax.post(a.urls.constructWriteUrl(b, d), {
							...
							error : function(b, c, d) {
								if (b == "not_found")
									f(new a.errors.FPError(121));
								else if (b == "bad_params")
									f(new a.errors.FPError(122));
								else if (b == "not_authorized")
									f(new a.errors.FPError(403));
								else
									f(new a.errors.FPError(123));
							},
							progress : g
						});
					};
					...
					var i = function(b, c, d, e, f) {
						...
						if (!c.filename)
							c.filename = b.value.replace("C:\\fakepath\\", "")
									|| b.name;
						var g = b.name;
						b.name = "fileUpload";
						a.iframeAjax.post(a.urls.constructStoreUrl(c), {
							...
							error : function(b, c, d) {
								if (b == "not_found")
									e(new a.errors.FPError(121));
								else if (b == "bad_params")
									e(new a.errors.FPError(122));
								else if (b == "not_authorized")
									e(new a.errors.FPError(403));
								else
					...
					var m = function(b, c, d, e, f) {
						...
						a.ajax.post(a.urls.constructStoreUrl(c), {
							...
							error : function(b, c, d) {
								if (b == "not_found")
									e(new a.errors.FPError(151));
								else if (b == "bad_params")
									e(new a.errors.FPError(152));
								else if (b == "not_authorized")
									e(new a.errors.FPError(403));
								else
									e(new a.errors.FPError(153));
							},
							progress : f
						});
					};
						...
						a.ajax.get(b + "/metadata", {
							...
							success : function(a) {
								for (var b = 0; b < f.length; b++)
									if (a[f[b]])
										a[f[b]] = new Date(a[f[b]]);
								d(a);
							},
							error : function(b, c, d) {
								if (b == "not_found")
									e(new a.errors.FPError(161));
								else if (b == "bad_params")
									e(new a.errors.FPError(400));
								else if (b == "not_authorized")
									e(new a.errors.FPError(403));
								else
									e(new a.errors.FPError(162));
							}
						});
					};
					var o = function(b, c, d, e) {
						...
						a.ajax.post(b + "/remove", {
							...
							error : function(b, c, d) {
								if (b == "not_found")
									e(new a.errors.FPError(171));
								else if (b == "bad_params")
									e(new a.errors.FPError(400));
								else if (b == "not_authorized")
									e(new a.errors.FPError(403));
								else
									e(new a.errors.FPError(172));
							}
						});
					};
					return {
		...
		.extend(
				...
				function() {
					...
					var h = function(e, f) {
						...
						e += (e.indexOf('?') >= 0 ? '&' : '?') + 'plugin='
								+ a.urls.getPlugin() + '&_cacheBust='
								+ a.util.getId();
						...
						l.target = b;
						var m = f.data;
						if (a.util.isFileInputElement(m) || a.util.isFile(m))
						...
						if (a.util.isFile(m)) {
							...
							if (!n)
								throw a
										.FilepickerException("Couldn't find corresponding file input.");
							m = {
								'fileUpload' : n
							};
							...
							m = {};
							m.fileUpload = o;
						} else if (m && a.util.isElement(m)
								&& m.tagName == "INPUT") {
							var o = m;
							m = {};
							m[o.name] = o;
						} else if (f.processData !== false)
							m = {
								'data' : m
							};
						m.format = 'iframe';
					...
					var j = function(a) {
						...
						for (var c = 0; c < b.length; c++) {
							var d = b[0];
							if (d.type != "file" || !d.files || !d.files.length)
								continue;
...
filepicker
		.extend(
				function() {
					...
					var d = function() {
						if (!a.apikey)
							throw new a.FilepickerException("API Key not found");
					};
					...
					var g = function(b, c, e, f, g) {
						d();
						if (!b || !c || typeof b === "function"
								|| typeof b === "function")
							throw new a.FilepickerException(
									"Not all required parameters given, missing picker or store options");
						...
						i.storeRegion = c.region;
						i.storeAccess = c.access || 'private';
						if (h && i.storePath)
							if (i.storePath.charAt(i.storePath.length - 1) != "/")
								throw new a.FilepickerException(
										"pickAndStore with multiple files requires a path that ends in '/'");
						var j = e;
						if (!h)
							j = function(a) {
								e([ a ]);
					...
					var i = function(b, c, e, f, g) {
						...
						if (!b)
							throw new a.FilepickerException(
									"No input given - nothing to read!");
						if (typeof c === "function") {
							g = f;
							f = e;
							e = c;
						...
						if (typeof b == "string")
							if (a.util.isFPUrl(b))
								a.files.readFromFPUrl(b, c, e, f, g);
							else
								a.files.readFromUrl(b, c, e, f, g);
						else if (a.util.isFileInputElement(b))
							if (!b.files)
								j(b, c, e, f, g);
							else if (b.files.length === 0)
								f(new a.errors.FPError(115));
							else
								a.files.readFromFile(b.files[0], c, e, f, g);
						else if (a.util.isFile(b))
							a.files.readFromFile(b, c, e, f, g);
						else if (b.url)
							a.files.readFromFPUrl(b.url, c, e, f, g);
						else
							throw new a.FilepickerException(
									"Cannot read given input: "
											+ b
											+ ". Not a url, file input, DOM File, or FPFile object.");
					};
					var j = function(b, c, d, e, f) {
						f(10);
					...
					var k = function(b, c, e, f, g, h) {
						...
						if (!b)
							throw new a.FilepickerException(
									"No fpfile given - nothing to write to!");
						if (c === undefined || c === null)
							throw new a.FilepickerException(
									"No input given - nothing to write!");
						...
						if (a.util.isFPUrl(b))
							...
							throw new a.FilepickerException(
									"Invalid file to write to: "
											+ b
											+ ". Not a filepicker url or FPFile object.");
						if (typeof c == "string")
							a.files.writeDataToFPUrl(i, c, e, f, g, h);
						else if (a.util.isFileInputElement(c))
							if (!c.files)
								a.files.writeFileInputToFPUrl(i, c, e, f, g, h);
							else if (c.files.length === 0)
								g(new a.errors.FPError(115));
							else
								a.files.writeFileToFPUrl(i, c.files[0], e, f,
										g, h);
						else if (a.util.isFile(c))
							a.files.writeFileToFPUrl(i, c, e, f, g, h);
						else if (c.url)
							a.files.writeUrlToFPUrl(i, c.url, e, f, g, h);
						else
							throw new a.FilepickerException(
									"Cannot read from given input: "
											+ c
											+ ". Not a string, file input, DOM File, or FPFile object.");
					};
					...
					var m = function(b, c, e, f) {
						...
						f = f || a.errors.handleError;
						var g;
						if (typeof b == "string" && a.util.isUrl(b))
							g = b;
						else if (b.url) {
							g = b.url;
							if (!c.mimetype && !c.extension)
								c.mimetype = b.mimetype;
							if (!c.suggestedFilename)
								c.suggestedFilename = b.filename;
						} else
							throw new a.FilepickerException(
									"Invalid file to export: "
											+ b
											+ ". Not a valid url or FPFile object. You may want to use filepicker.store() to get an FPFile to export");
						if (c.suggestedFilename)
							c.suggestedFilename = encodeURI(c.suggestedFilename);
					...
					var n = function(b, c, e, f, g) {
						...
						if (typeof b == "string")
							a.files.storeData(b, c, e, f, g);
						else if (a.util.isFileInputElement(b))
							if (!b.files)
								a.files.storeFileInput(b, c, e, f, g);
							else if (b.files.length === 0)
								f(new a.errors.FPError(115));
							else
								a.files.storeFile(b.files[0], c, e, f, g);
						else if (a.util.isFile(b))
							a.files.storeFile(b, c, e, f, g);
						else if (b.url) {
							if (!c.filename)
					...
					var p = function(b, c, e, f) {
						...
						if (a.util.isFPUrl(b))
							...
							throw new a.FilepickerException(
									"Invalid file to get metadata for: "
											+ b
											+ ". Not a filepicker url or FPFile object.");
					...
					var r = function(b, c, e, f, g, h) {
						...
						if (!b)
							throw new a.FilepickerException(
									"No fpfile given - nothing to convert!");
						if (typeof e === "function") {
							h = g;
							g = f;
							f = e;
						...
						if (e.path)
							options.storePath = e.path;
						if (e.container)
							options.storeContainer = e.container;
						if (e.region)
						...
						if (a.util.isFPUrl(b))
							...
							i = b.url;
							if (!a.mimetypes.isSupportedMimetype(b.mimetype)) {
								g(new a.errors.FPError(142));
								return;
							}
						} else
							throw new a.FilepickerException(
									"Invalid file to convert: "
											+ b
											+ ". Not a filepicker url or FPFile object.");
		...
		.extend(
				...
				function() {
					...
					var c = [
							...
							'application/psd', 'application/x-photoshop',
							'image/photoshop', 'image/psd',
							'image/x-photoshop', 'image/x-psd',
							'image/vnd.adobe.photoshop', 'text/html',
							'text/plain' ];
					var d = [ 'application/octet-stream',
							'application/download',
							'application/force-download', 'octet/stream',
							'application/unknown', 'application/x-download',
					...
					var f = function(b, c) {
						...
						c = a.util.trim(c).toLowerCase();
						for (var e = 0; e < d.length; e++)
							if (b == d[e])
								return true;
						test_parts = b.split("/");
						against_parts = c.split("/");
						if (against_parts[0] == "*")
							return true;
		...
		.extend(
				...
				function() {
					...
					var f = function(b) {
						...
						d.style.padding = "0";
						d.style.background = '#ecedec url("https://www.filepicker.io/static/img/spinner.gif") no-repeat 50% 50%';
						d.style.top = '10px';
						var e = a.window.getSize(), f;
						if (b) {
							d.style.top = '0';
							d.style.right = '0';
							d.style.bottom = '0';
							...
							var g = Math.min(e[1] - 40, 540), f = Math.min(
									e[0] - 40, 860), h = (e[0] - f - 40) / 2;
							d.style.left = h + "px";
							d.style.top = '10px';
							d.style.bottom = 'auto';
							d.style.right = 'auto';
					...
					var g = function(a) {
						...
						b.style.padding = '4px 10px 6px 10px';
					...
					var h = function(d, e) {
						...
						return function() {
							if (a.uploading && !e)
								if (!window
										.confirm("You are currently uploading. If you choose 'OK', the window will close and your upload will not finish. Do you want to stop uploading and close the window?"))
									return;
							a.uploading = false;
				...
				function() {
					...
					var b = function(b) {
						...
						if (b.services)
							for (var d = 0; d < b.services.length; d++) {
								var e = ('' + b.services[d]).replace(" ", "");
								if (a.services[e] !== undefined)
									e = a.services[e];
								b.services[d] = e;
							}
						if (b.mimetypes && b.extensions)
							throw a
					...
					var c = function(b, c) {
						var d = function(d) {
							...
							if (d.error) {
								...
								e.mimetype = d.payload.data.type;
								e.size = d.payload.data.size;
								if (d.payload.data.key)
									e.key = d.payload.data.key;
								if (d.payload.data.container)
									e.container = d.payload.data.container;
								if (d.payload.data.path)
									e.path = d.payload.data.path;
								e.isWriteable = true;
								b(e);
							}
							a.modal.close();
					...
					var h = function(f, i, j, k, l, m) {
						...
						if (f.debug) {
							...
							if (k || f.storeLocation)
								dumy_callback = [ dumy_data, dumy_data,
										dumy_data ];
							else
		...
		.extend(
				...
				function() {
					...
					var e = function(a) {
						...
						var b = document.createElement('a');
						b.href = a;
						var c = b.hostname.indexOf(":") == -1 ? b.hostname
								: b.host;
						var d = {
							source : a,
							...
							query : b.search,
							params : (function() {
								var a = {}, c = b.search.replace(/^\?/, '')
										.split('&'), d = c.length, e = 0, f;
								for (var e = 0; e < d; e++) {
									if (!c[e])
										continue;
									f = c[e].split('=');
...
filepicker.extend("urls", function() {
	...
	var h = function(b, c, e) {
		return d
				...
				+ (b.container !== undefined ? "&container=" + b.container
						: "modal")
				+ (e ? "&multi=" + !!e : "")
				+ (b.mimetypes !== undefined ? "&m=" + b.mimetypes.join(",")
						: "")
				+ (b.extensions !== undefined ? "&ext="
						+ b.extensions.join(",") : "")
				+ (b.openTo !== undefined ? "&loc=" + b.openTo : "")
				+ '&language='
				+ (b.language || a.browser.getLanguage())
				+ (b.maxSize ? "&maxSize=" + b.maxSize : "")
				+ (b.maxFiles ? "&maxFiles=" + b.maxFiles : "")
				+ (b.signature ? "&signature=" + b.signature : "")
				+ (b.policy ? "&policy=" + b.policy : "")
				+ (b.mobile !== undefined ? "&mobile=" + b.mobile : "")
				+ (b.folders !== undefined ? "&folders=" + b.folders : "")
				+ (b.storeLocation ? "&storeLocation=" + b.storeLocation : "")
				+ (b.storePath ? "&storePath=" + b.storePath : "")
				+ (b.storeContainer ? "&storeContainer=" + b.storeContainer
						: "")
				+ (b.storeRegion ? "&storeRegion=" + b.storeRegion : "")
				+ (b.storeAccess ? "&storeAccess=" + b.storeAccess : "")
				+ "&plugin=" + n();
	};
	var i = function(b, c) {
		return f
				...
				+ (b.container !== undefined ? "&container=" + b.container
						: "modal")
				+ (b.openTo !== undefined ? "&loc=" + b.openTo : "")
				+ '&language=' + (b.language || a.browser.getLanguage())
				+ (b.signature ? "&signature=" + b.signature : "")
				+ (b.policy ? "&policy=" + b.policy : "")
				+ (b.mobile !== undefined ? "&mobile=" + b.mobile : "")
				+ "&plugin=" + n();
	...
	var j = function(b, c, d) {
		...
		return e
				...
				+ "&version="
				+ a.API_VERSION
				+ (c.services ? "&s=" + c.services.join(",") : "")
				+ (c.container !== undefined ? "&container=" + c.container
						: "modal")
				+ (c.openTo !== undefined ? "&loc=" + c.openTo : "")
				+ (c.mimetype !== undefined ? "&m=" + c.mimetype : "")
				+ (c.extension !== undefined ? "&ext=" + c.extension : "")
				+ '&language='
				+ (c.language || a.browser.getLanguage())
				+ (c.mobile !== undefined ? "&mobile=" + c.mobile : "")
				+ (c.suggestedFilename !== undefined ? "&defaultSaveasName="
						+ c.suggestedFilename : "")
				+ (c.signature ? "&signature=" + c.signature : "")
				+ (c.policy ? "&policy=" + c.policy : "") + "&plugin=" + n();
	};
	var k = function(b) {
		return g
				+ b.location
				+ "?key="
				+ a.apikey
				+ (b.base64decode ? "&base64decode=true" : "")
				+ (b.mimetype ? "&mimetype=" + b.mimetype : "")
				+ (b.filename ? "&filename=" + encodeURIComponent(b.filename)
						: "") + (b.path ? "&path=" + b.path : "")
				+ (b.container ? "&container=" + b.container : "")
				+ (b.region ? "&region=" + b.region : "")
				+ (b.access ? "&access=" + b.access : "")
				+ (b.signature ? "&signature=" + b.signature : "")
				+ (b.policy ? "&policy=" + b.policy : "") + "&plugin=" + n();
	};
	var l = function(a, b) {
		return a + "?nonce=fp" + (!!b.base64decode ? "&base64decode=true" : "")
				+ (b.mimetype ? "&mimetype=" + b.mimetype : "")
				+ (b.signature ? "&signature=" + b.signature : "")
				+ (b.policy ? "&policy=" + b.policy : "") + "&plugin=" + n();
	};
...
filepicker.extend("util", function() {
	...
	var d = function(a) {
		if (typeof HTMLElement === "object")
			...
			return a && typeof a === "object" && a.nodeType === 1
	...
	var k = function(a) {
		return function() {
			if (window.console && typeof window.console[a] == "function")
				try {
		...
		.extend(
				...
				function() {
					...
					var d = function(d) {
						...
						e.innerHTML = d.getAttribute('data-fp-button-text')
								|| d.getAttribute('data-fp-text')
								|| "Pick File";
						e.className = d.getAttribute('data-fp-button-class')
								|| d.getAttribute('data-fp-class')
								|| d.className || 'fp__btn';
						d.style.display = "none";
						var f = {};
						...
						b("storeRegion", f, "data-fp-store-region", d);
						b("storeAccess", f, "data-fp-store-access", d);
						b("language", f, "data-fp-language", d);
						var g = d.getAttribute("data-fp-services");
						if (!g)
							g = d.getAttribute("data-fp-option-services");
						if (g) {
							g = g.split(",");
							for (var h = 0; h < g.length; h++)
								g[h] = a.services[g[h].replace(" ", "")]
										|| g[h];
							f.services = g;
						}
						var i = d.getAttribute("data-fp-service");
						if (i)
							f.service = a.services[i.replace(" ", "")] || i;
						if (f.mimetypes)
							f.mimetypes = f.mimetypes.split(",");
						if (f.extensions)
							f.extensions = f.extensions.split(",");
						var j = d.getAttribute("data-fp-apikey");
						if (j)
							a.setKey(j);
						f.folders = (d.getAttribute("data-fp-folders") || "false") == "true";
						var k = (d.getAttribute("data-fp-multiple")
								|| d.getAttribute("data-fp-option-multiple") || "false") == "true";
						if (k)
							e.onclick = function() {
								e.blur();
					...
					var e = function(d) {
						...
						e.className = d.getAttribute('data-fp-class')
								|| d.className;
						e.style.padding = "1px";
						d.style.display = "none";
						d.parentNode.insertBefore(e, d.nextSibling);
						...
						i.innerHTML = d.getAttribute('data-fp-button-text')
								|| "Pick File";
						...
						g(j);
						j.innerHTML = d.getAttribute('data-fp-drag-text')
								|| "Or drop files here";
						...
						b("storeRegion", k, "data-fp-store-region", d);
						b("storeAccess", k, "data-fp-store-access", d);
						b("language", k, "data-fp-language", d);
						var l = d.getAttribute("data-fp-services");
						if (!l)
							l = d.getAttribute("data-fp-option-services");
						if (l) {
							l = l.split(",");
							for (var m = 0; m < l.length; m++)
								l[m] = a.services[l[m].replace(" ", "")]
										|| l[m];
							k.services = l;
						}
						var n = d.getAttribute("data-fp-service");
						if (n)
							k.service = a.services[n.replace(" ", "")] || n;
						if (k.mimetypes)
							k.mimetypes = k.mimetypes.split(",");
						if (k.extensions)
							k.extensions = k.extensions.split(",");
						var o = d.getAttribute("data-fp-apikey");
						if (o)
							a.setKey(o);
						var p = (d.getAttribute("data-fp-multiple")
								|| d.getAttribute("data-fp-option-multiple") || "false") == "true";
						if (a.dragdrop.enabled())
							h(j, p, k, d);
						else
							j.innerHTML = "&nbsp;";
						if (p)
							j.onclick = i.onclick = function() {
								i.blur();
					...
					var f = function(b, d, e) {
						...
						var h = function(e) {
							...
							if (!a.dragdrop.enabled)
								...
								d.innerHTML = b
										.getAttribute('data-fp-drag-text')
										|| "Or drop files here";
							b.value = '';
							c(b);
							return false;
										...
										b,
										{
											...
											onError : function(a, c) {
												if (a == "TooManyFiles")
													...
													b.innerHTML = c;
												else if (a == "NoFilesFound")
													b.innerHTML = c;
												else if (a == "UploadError")
													b.innerHTML = "Oops! Had trouble uploading.";
											},
											onStart : function(a) {
												j = i(b);
											},
					...
					var j = function(c) {
						c.onclick = function() {
							...
							b("suggestedFilename", e,
									"data-fp-option-defaultSaveasName", c);
							b("container", e, "data-fp-container", c);
							b("suggestedFilename", e,
									"data-fp-suggestedFilename", c);
							b("mimetype", e, "data-fp-mimetype", c);
							b("extension", e, "data-fp-extension", c);
							var f = c.getAttribute("data-fp-services");
							if (!f)
								f = c.getAttribute("data-fp-option-services");
							if (f) {
								f = f.split(",");
								for (var g = 0; g < f.length; g++)
									f[g] = a.services[f[g].replace(" ", "")]
											|| f[g];
								e.services = f;
							}
							var h = c.getAttribute("data-fp-service");
							if (h)
								e.service = a.services[h.replace(" ", "")] || h;
					...
					var k = function() {
						if (document.querySelectorAll) {
							var a;
							var b = document
									.querySelectorAll('input[type="filepicker"]');
							for (var a = 0; a < b.length; a++)
								d(b[a]);
							var c = document
									.querySelectorAll('input[type="filepicker-dragdrop"]');
							for (var a = 0; a < c.length; a++)
								e(c[a]);
							var f = [];
							var g = document
									.querySelectorAll('button[data-fp-url]');
							...
							for (a = 0; a < g.length; a++)
								f.push(g[a]);
							g = document
									.querySelectorAll('input[type="button"][data-fp-url]');
							for (a = 0; a < g.length; a++)
								f.push(g[a]);
							for (a = 0; a < f.length; a++)
								j(f[a]);
							var h = document
									.querySelectorAll('[type="filepicker-preview"][data-fp-url]');
					...
					var l = function(a) {
						...
						if (b == 'filepicker')
							d(a);
						else if (b == 'filepicker-dragdrop')
							e(a);
						else
							j(a);
					};
					var m = function(b) {
						var c = b.getAttribute("data-fp-url");
						if (!c || !a.util.isFPUrl(c))
							return true;
						else
							c = c.replace('api/file/', 'api/preview/');
...
filepicker
		.extend(
				...
				function() {
					...
					var g = function() {
						var b = (a.browser.isIOS() || a.browser.isAndroid()), c, d;
						if (b && a.browser.isMobileSafari()) {
							...
							c = f()[0] < 768;
							d = a.cookies.THIRD_PARTY_COOKIES === false;
						}
						return (c || d || b);
					};
						...
						if (b == 'window') {
							...
							if (!o)
								throw new a.FilepickerException(
										"Container '"
												+ b
												+ "' not found. This should either be set to 'window','modal', or the ID of an iframe that is currently in the document.");
							o.src = h;
						}
					};
					return {
