...
//============================ Controller  ===================================-->
//============================================================================-->
public class Edit_Customer_Request_Controller extends HttpServlet {
							...
							for (int k = 1; k <= bean.getSrno(); k++) {
								...
								if (fieldName.equalsIgnoreCase("inputName" + k)) {
									...
									if (k >= 1) {
										...
										flag = bo.EditchangeRequest(bean,
												app_list, chang_dept, session);
									}
									// Attach file ====>
									bean.setFile_blob(file_Input);
									if (bean.getFile_Name() != "") {
										flag = dao.Editattach_File(bean,
												session);
									}
