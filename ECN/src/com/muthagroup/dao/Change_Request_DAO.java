...
public class Change_Request_DAO {
	...
	public boolean registerChange(change_Request_Vo bean, ArrayList app_list, ArrayList chang_list, HttpSession session, ArrayList trk_list) {
		boolean flag = false;
		...
		try {
			...
			if (complaint_no.equalsIgnoreCase("Related Complaint No(If any)")) {
				System.out.println("c =  in loop" + bean.getItem_Id());
				PreparedStatement ps_change = con.prepareStatement("insert into cr_tbl(Company_Id,Item_Id,Present_System,Proposed_System,Objective,CR_Date,Proposed_Impl_Date,Actual_Impl_Date,CR_Status_Id,U_Id,mail_status)values(?,?,?,?,?,?,?,?,?,?,?) ");
				...
				PreparedStatement ps_change = con.prepareStatement("insert into cr_tbl(Company_Id,Item_Id,Present_System,Proposed_System,Objective,CR_Date,Proposed_Impl_Date,Actual_Impl_Date,Complaint_No,CR_Status_Id,U_Id,mail_status)values(?,?,?,?,?,?,?,?,?,?,?,?) ");
				ps_change.setInt(1, bean.getCompany_Id());
				ps_change.setInt(2, bean.getItem_Id());
				ps_change.setString(3, bean.getPresent_System());
				ps_change.setString(4, bean.getProposed_System());
			...
			for (int tr = 0; tr < trk_list.size(); tr++) {
				ps_tr = con.prepareStatement("select * from cr_tracking_change where TC_Type='" + trk_list.get(tr).toString() + "'");
			...
			for (int trset = 0; trset < listtr.size(); trset++) {
				PreparedStatement ps_trcg = con.prepareStatement("insert into cr_tc_rel_tbl(CR_No,TC_Id)values(?,?)");
				ps_trcg.setInt(1, crno);
				ps_trcg.setInt(2, Integer.parseInt(listtr.get(trset).toString()));
			...
			for (int f = 0; f < app_list.size(); f++) {
				PreparedStatement ps_approver = con.prepareStatement("insert into cr_approver_relation_tbl(CR_No,U_Id)values(?,?)");
			...
			for (int ch = 0; ch < chang_list.size(); ch++) {
				PreparedStatement pstype_change = con.prepareStatement("select * from cr_tbl_type where CR_Type='" + chang_list.get(ch).toString() + "'");
			...
			for (java.util.Iterator itr = categoryList.iterator(); itr.hasNext();) {
				if (itr.next() == null) {
					itr.remove();
				}
			}
			...
			for (int list = 0; list < categoryList.size(); list++) {
				category_var = Integer.valueOf(categoryList.get(list).toString());
			//	System.out.println("Category List = " + categoryList.get(list).toString());
				ps_category = con.prepareStatement("insert into cr_category_relation_tbl(CR_No,CR_Category_Id)values(?,?)");
				ps_category.setInt(1, crno);
				ps_category.setInt(2, category_var);
			...
			if (complaint_no.equalsIgnoreCase("Related Complaint No(If any)")) {
				...
				PreparedStatement ps_ctype_hist = con.prepareStatement("insert into cr_tbl_hist(CR_Hist_Date,CR_No,Company_Id,Item_Id,Present_System,Proposed_System,Objective,CR_Date,Proposed_Impl_Date,Actual_Impl_Date,U_Id)values(?,?,?,?,?,?,?,?,?,?,?)");
			...
			PreparedStatement ps_CR_T_Rel_Id = con.prepareStatement("select * from cr_ctype_relation_tbl where CR_No=" + crno);
			...
			for (int hist = 0; hist < hist_ctypeID.size(); hist++) {
				...
				PreparedStatement cr_ctype_hist = con.prepareStatement("insert into cr_ctype_relation_tbl_hist(CR_T_Rel_Hist_Date,CR_No,CR_Type_Id)values(?,?,?)");
				cr_ctype_hist.setTimestamp(1, timestamp);
				cr_ctype_hist.setInt(2, crno);
				cr_ctype_hist.setInt(3, Integer.parseInt(hist_ctypeID.get(hist).toString())); 
			...
			PreparedStatement ps_category_hist = con.prepareStatement("select * from cr_category_relation_tbl where CR_No=" + crno);
			...
			for (int cathis = 0; cathis < hiscat11.size(); cathis++) { 
				PreparedStatement ps_cathisrel = con.prepareStatement("insert into cr_category_relation_tbl_hist(CR_C_Rel_Hist_Date,CR_No,CR_Category_Id)values(?,?,?)");
				ps_cathisrel.setTimestamp(1, timestamp);
				ps_cathisrel.setInt(2, crno);
				ps_cathisrel.setInt(3, Integer.parseInt(hiscat11.get(cathis).toString()));
			...
			PreparedStatement ps_approver = con.prepareStatement("select * from cr_approver_relation_tbl where CR_No=" + bean.getCrno());
			...
			for (int appr = 0; appr < list_approver11.size(); appr++) {
				PreparedStatement ps_approv = con.prepareStatement("insert into cr_approver_relation_tbl_hist(CR_A_Rel_Hist_Date,CR_No,U_Id)values(?,?,?)");
				ps_approv.setTimestamp(1, timestamp);
				ps_approv.setInt(2, crno);
				ps_approv.setInt(3, Integer.parseInt(list_approver11.get(appr).toString())); 
			...
			//System.out.println("USER ID... :"+ Integer.parseInt(session.getAttribute("uid").toString()));
			...
			for (int appr = 0; appr < list_approver11.size(); appr++) {
				...
				ps_approv = con.prepareStatement("insert into cr_tbl_approval(CR_No,U_Id,Approval_Id)values(?,?,?)"); 
				ps_approv.setInt(1, cr_no);
				ps_approv.setInt(2,Integer.parseInt(list_approver11.get(appr).toString()));
				ps_approv.setInt(3, approval_id);

				approval = ps_approv.executeUpdate();

				PreparedStatement ps_approv_hist = con.prepareStatement("insert into cr_tbl_approval_hist(CR_Approval_Date_Hist,U_Id,CR_No,Approval_Id)values(?,?,?,?)");
				ps_approv_hist.setTimestamp(1, timestamp);
				ps_approv_hist.setInt(2, Integer.parseInt(list_approver11.get(appr).toString()));
			...
			for (int ccat = 0; ccat < categoryList.size(); ccat++) {
				ps_approvers = con.prepareStatement("select * from cr_tbl_category where CR_Category_Id=" + categoryList.get(ccat).toString());
			...
			String comp = null;
			PreparedStatement ps_comp = con.prepareStatement("select * from user_tbl_company where Company_Id=" + bean.getCompany_Id());
			ResultSet rs_comp = ps_comp.executeQuery();
			while (rs_comp.next()) {
				comp = rs_comp.getString("Company_Name");
			}
			String Part = null;
			PreparedStatement ps_part = con.prepareStatement("select * from customer_tbl_item where Item_Id=" + bean.getItem_Id());
			ResultSet rs_part = ps_part.executeQuery();
			while (rs_part.next()) {
				Part = rs_part.getString("Item_Name");
			}
			String Name = null;
			PreparedStatement ps_uid = con.prepareStatement("select * from user_tbl where U_Id=" + Integer.parseInt(session.getAttribute("uid").toString()));
			...
			for (int p = 0; p < app_list.size(); p++) {
				PreparedStatement ps_p = con.prepareStatement("select * from user_tbl where U_Id=" + Integer.parseInt(app_list.get(p).toString()));
			...
			// multiple recipients : == >
			// *********************************************************************************************
			/*PreparedStatement ps6 = con.prepareStatement("select count(Email) from automail where Company_id=6 or company_id=" + bean.getCompany_Id());
			ResultSet rs6 = ps6.executeQuery();
			while (rs6.next()) {
				count = rs6.getInt("count(Email)");
			}

			PreparedStatement ps_auto = con.prepareStatement("select * from automail where Company_id=6 or company_id=" + bean.getCompany_Id());
			...
			ps_uid = con.prepareStatement("select * from user_tbl where U_Id=" + Integer.parseInt(session.getAttribute("uid").toString()));
			rs_uid = ps_uid.executeQuery();
			while (rs_uid.next()) {
				j++;
				System.out.println("Size List = " + j);
				recipients[j] = rs_uid.getString("U_Email");
			}*/
			...
			try { 
				...
				if (transport.equals("")) {
					...
					PreparedStatement ps_mailcheck = con.prepareStatement("update cr_tbl set mail_status=0 where CR_No=" + crno);
	...
	public boolean attach_File(change_Request_Vo bean, HttpSession session) {
		...
		try {
			...
			ps_file = con.prepareStatement("insert into cr_tbl_attachment(CR_No,Attachment,Attach_Date,U_Id,File_Name,Del_Status)values(?,?,?,?,?,?)");
			...
			PreparedStatement ps_del = con.prepareStatement("delete from cr_tbl_attachment where File_Name='"+ "" + "'");
			int d = ps_del.executeUpdate();

			PreparedStatement ps_history = con.prepareStatement("insert into cr_tbl_attachment_hist(Attach_Date_HIst,CR_No,Attach_Date_original,U_Id,File_Name,Del_Status)values(?,?,?,?,?,?)");
			...
			PreparedStatement ps_del1 = con.prepareStatement("delete from cr_tbl_attachment_hist where File_Name='" + "" + "'");
