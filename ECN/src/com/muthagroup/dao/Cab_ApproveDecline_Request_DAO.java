...
public class Cab_ApproveDecline_Request_DAO {
	...
	public boolean registerApproval(Cab_ApproveDecline_Request_Vo bean, HttpSession session, ArrayList appsellist) {
		int approval_type = 0;
		int cr_no = 0;
		String remark = null;
		int uid = 0;
		...
		try {
			...
			if (appsellist.size() != 0) {
				...
				/*if (newuidapp.size() > 0) {
					PreparedStatement ps_deleteOrgApprovers = con.prepareStatement("delete from cr_approver_relation_tbl where cr_no=" + cr_no);
					...
					for (int appr = 0; appr < newuidapp.size(); appr++) {
						...
						ps_approver.setInt(2, Integer.parseInt(newuidapp.get(appr).toString()));
					...
					for (int appr1 = 0; appr1 < newuidapp.size(); appr1++) {
						...
						ps_approv.setInt(3, Integer.parseInt(newuidapp.get(appr1).toString()));
			...
			PreparedStatement ps_AppUid = con.prepareStatement("select * from cr_tbl_approval where CR_No=" + cr_no);
			...
			while (rsAppUid.next()) {
				for (int s = 0; s < id1.size(); s++) {
					if (rsAppUid.getInt("U_Id") == Integer.parseInt(id1.get(s).toString())) {
						...
						PreparedStatement ps_approve = con.prepareStatement("update cr_tbl_approval set Approval_Id=?,Remark=?,CR_Approval_Date=? where cr_no="
										+ cr_no
										+ " and U_Id="
						...
						PreparedStatement ps_date = con.prepareStatement("select * from cr_tbl_approval where cr_no="
										+ cr_no
										+ " and U_Id="
										+ Integer.parseInt(id1.get(s).toString()));
						ResultSet rs_date = ps_date.executeQuery();
						while (rs_date.next()) {
							original_date = rs_date.getTimestamp("CR_Approval_Date");
							// listApprovee.add(rs_date.getInt("Approval_Id"));
							// System.out.println("Approveeeeee ==== " +
							// listApprovee);
						}

						PreparedStatement ps_getU_Id = con.prepareStatement("select U_Id from CR_Tbl_Approval where cr_No=" + cr_no);
						...
						for (int ss = 0; ss < ExtApprovers.size(); ss++) {
							PreparedStatement ps_UName = con.prepareStatement("select U_Name from User_Tbl where U_Id="
											+ Integer.parseInt(ExtApprovers.get(ss).toString()));
							ResultSet rs_UName = ps_UName.executeQuery();

							while (rs_UName.next()) {
								ExtApproversNames.add(rs_UName.getString("U_Name"));
							}

						}

						if (appsellist.size() != 0 && ExtApproversNames.size() != 0) {
							for (int a1 = 0; a1 < ExtApproversNames.size(); a1++) {
								for (int b1 = 0; b1 < appsellist.size(); b1++) {
									if (ExtApproversNames.contains(appsellist.get(b1))) {
						...
						//System.out.println("************************* selected approver list.... " + appsellist);
						//System.out.println("************************* Extra selected approver list.... " + ExtApproversNames);

						for (int dd = 0; dd < appsellist.size(); dd++) {
							ps_getUId = con.prepareStatement("select max(U_Id) from User_Tbl where U_name='" + appsellist.get(dd).toString() + "'"); 
							...
							while (rs_getUId.next()) {
								...
								ps_addUId = con.prepareStatement("insert into cr_tbl_approval(U_Id,CR_No,Approval_Id)values(?,?,?)");
						...
						PreparedStatement ps_approve_hist = con.prepareStatement("insert into cr_tbl_approval_hist(CR_Approval_Date_Hist,U_Id,CR_No,Approval_Id,Remark,CR_Status_Id,CR_Approval_Date)values(?,?,?,?,?,?,?)");
						...
						PreparedStatement ps_crtbl = con.prepareStatement("select * from cr_tbl where CR_No=" + cr_no);
						...
						while (rs_crtbl.next()) {
							...
							PreparedStatement ps_part = con.prepareStatement("select * from customer_tbl_item where Item_Id="
						...
						PreparedStatement ps_crtblapproval = con.prepareStatement("select * from cr_tbl_approval where CR_No=" + cr_no);
						...
						for (int ap = 0; ap < app_uid.size(); ap++) {
							PreparedStatement ps_appr = con.prepareStatement("select * from user_tbl where U_Id=" + Integer.parseInt(app_uid.get(ap).toString()));
						...
						PreparedStatement ps_category = con.prepareStatement("select * from cr_category_relation_tbl where CR_No=" + cr_no);
						ResultSet rs_category = ps_category.executeQuery();
						while (rs_category.next()) {
							categoryID.add(rs_category.getInt("CR_Category_Id"));
						}

						for (int a = 0; a < categoryID.size(); a++) {
							PreparedStatement ps_cat = con.prepareStatement("select * from cr_tbl_category where CR_Category_Id=" + Integer.parseInt(categoryID.get(a).toString()));
						...
						PreparedStatement ps_ctype = con.prepareStatement("select * from cr_ctype_relation_tbl where CR_No=" + cr_no);
						...
						for (int b = 0; b < ctypeId.size(); b++) {
							PreparedStatement ps_ct = con.prepareStatement("select * from cr_tbl_type where CR_Type_Id=" + Integer.parseInt(ctypeId.get(b).toString()));
						...
						if (approval.equalsIgnoreCase("Pending")) {
							...
							// recipients[tt1]);
							// }
							//System.out.println("Pending Approvals = " + email);
							// *********************************************************************************************
							try {
								...
								msg.setContent("<b style='color: #A69514;'><blink>Change "
												+ approval
												+ "</b></blink>"
							...
							//System.out.println("Approved in looop ............");
							...
							PreparedStatement ps_Topauto = con.prepareStatement("select * from automail");
							ResultSet rs_Topauto = ps_Topauto.executeQuery();
							while (rs_Topauto.next()) {topEmail.add(rs_Topauto.getString("Email"));
							}*/
							...
							/*for (int tt1 = 0; tt1 < recipients.length; tt1++) {
								System.out.println("Recipients ====== " + recipients[tt1]);
							}*/
