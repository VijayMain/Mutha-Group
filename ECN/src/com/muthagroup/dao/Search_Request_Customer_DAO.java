...
//============================ Data Access Model =============================-->
//============================================================================-->
public class Search_Request_Customer_DAO {

	boolean flag = false;
	ArrayList ListDisplay = new ArrayList();
	public boolean searchRequestDAO(Search_Request_Customer_Vo bean,
			...
			for (int s = 0; s < id1.size(); s++) {
				...
				if (bean.getEnd_date_sup() != null
					...
					PreparedStatement ps = con
							.prepareStatement("select * from crc_tbl where U_Id="
									+ Integer.parseInt(id1.get(s).toString())
									+ " and CRC_Date between '"
					...
					rs = ps.executeQuery();
						...
						&& bean.getCompany_name_sup() == 0) {
					PreparedStatement ps = con
							.prepareStatement("select * from crc_tbl where Company_Id="
					...
					rs = ps.executeQuery();
						...
						&& bean.getItem_name_item() == 0) {
					PreparedStatement ps = con
							.prepareStatement("select * from crc_tbl_approval where Approval_Id="
									+ bean.getApproval_type_app()
									+ " and U_Id="
									+ Integer.parseInt(id1.get(s).toString())
									+ " and CRC_Approval_Date between '"
					...
					rs = ps.executeQuery();
						...
						&& bean.getCompany_name_sup() == 0) {
					PreparedStatement ps = con
							.prepareStatement("select * from crc_tbl where Item_Id="
									+ bean.getItem_name_item()
									+ " and U_Id="
									+ Integer.parseInt(id1.get(s).toString()));
					rs = ps.executeQuery();
				...
				while (rs.next()) {
					ListDisplay.add(rs.getInt("CRC_No"));
				}
			}
