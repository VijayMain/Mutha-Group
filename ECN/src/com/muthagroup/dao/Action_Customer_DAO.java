...
//============================================================================-->
//============================ Customer Action Data Access Model =============================-->
//============================================================================-->
public class Action_Customer_DAO {
	...
	public int addAction1(Action_Customer_VO bean, HttpSession session) {
		int ActionId = 0;
		try {
			...
			ps_addAction = con
					.prepareStatement("insert into crc_tbl_action(CRC_No,Action_Discription,U_Id,Proposed_Date,Actual_Impl_Date,Action_Date,proposed_Output,Actual_Output)values(?,?,?,?,?,?,?,?)");
			...
			ps_addAction_hist = con
					.prepareStatement("insert into crc_tbl_action_hist(CRC_No,Action_Discription,U_Id,Proposed_Date,Actual_Impl_Date,Action_Date_Original,Acton_Date_Hist,proposed_Output,Actual_Output)values(?,?,?,?,?,?,?,?,?)");
			...
			PreparedStatement ps_ActionId = con
					.prepareStatement("select max(CRC_Action_Id) from crc_tbl_action");

			ResultSet rs_ActionId = ps_ActionId.executeQuery();

			while (rs_ActionId.next()) {
				ActionId = rs_ActionId.getInt("max(CRC_Action_Id)");
	...
	//======================= Attach File =================================-->
	//============================================================================-->
	public void attach_File1(Action_Customer_VO bean, HttpSession session) {
		try {
			...
			ps_file = con
					.prepareStatement("insert into crc_tbl_action_attachment(CRC_Action_Id,CRC_Action_Attachment,CRC_Action_Attach_Date,CRC_Action_delete_status,CRC_Action_File_Name,crc_no)values(?,?,?,?,?,?)");
			...
			PreparedStatement ps_del = con
					.prepareStatement("delete from crc_tbl_action_attachment where CRC_Action_File_Name='"
							+ "" + "'");
			ps_del.executeUpdate();

			PreparedStatement ps_attsel = con
					.prepareStatement("select * from crc_tbl_action_attachment where CRC_Action_Id="
							+ bean.getActionId());
			ResultSet rsattsel = ps_attsel.executeQuery();
			while (rsattsel.next()) {
				attid = rsattsel.getInt("CRC_Attach_Id");
			}
			// ****************************************************************************************

			PreparedStatement ps_history = con
					.prepareStatement("insert into crc_tbl_action_attachment_hist(CRC_Attach_Date_Hist,CRC_Attach_Date_Original,CRC_Action_Id,Delete_Status,File_name,CRC_No,CRC_Attach_Id) values(?,?,?,?,?,?,?) ");
			...
			ps_history.setInt(6,
					Integer.parseInt(session.getAttribute("crno").toString()));
			ps_history.setInt(7, attid);
			int l = ps_history.executeUpdate();

			PreparedStatement ps_del1 = con
					.prepareStatement("delete from crc_tbl_action_attachment_hist where File_Name='"
							+ "" + "'");
	...
	//========================= Add Actual Output ==============================-->
	//============================================================================-->
	public int add_ActualOP_dao1(Action_Customer_VO bean, HttpSession session) {
		int action_id = 0;
		int i = 0;
		...
		try {
			PreparedStatement ps_addActOP = con
					.prepareStatement("Update crc_tbl_action set Actual_Impl_Date=?,Actual_Output=? where CRC_Action_Id="
							+ bean.getAction_No());
			...
			if (i > 0) {
				PreparedStatement ps_getActioDetails = con
						.prepareStatement("select * from crc_tbl_action where CRC_Action_Id="
								+ bean.getAction_No());

				ResultSet rs_getActioDetails = ps_getActioDetails
						.executeQuery();
				while (rs_getActioDetails.next()) {
					cr_No = rs_getActioDetails.getInt("CRC_No");
					action_Disc = rs_getActioDetails
							.getString("Action_Discription");
					u_id = rs_getActioDetails.getInt("U_Id");
					proposed_Date = rs_getActioDetails
							.getTimestamp("Proposed_Date");
					actual_impl_Date = rs_getActioDetails
							.getTimestamp("Actual_Impl_Date");
					Action_Date_Original = rs_getActioDetails
							.getTimestamp("Action_Date");
					Prop_output = rs_getActioDetails
							.getString("proposed_Output");
					Actual_Output = rs_getActioDetails
							.getString("Actual_Output");
				}
				PreparedStatement ps_addAction_histOP = con
						.prepareStatement("insert into cr_tbl_action_hist(CRC_No,Action_Discription,U_Id,Proposed_Date,Actual_Impl_Date,Action_Date_Original,Acton_Date_Hist,proposed_Output,Actual_Output)values(?,?,?,?,?,?,?,?,?)");
