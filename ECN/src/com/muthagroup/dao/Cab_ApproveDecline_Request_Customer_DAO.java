...
public class Cab_ApproveDecline_Request_Customer_DAO {
	...
	public boolean registerApproval(
		...
		try {
			...
			if (appsellist.size() != 0) {
				...
				for (int a = 0; a < newuidapp.size(); a++) {
					...
					PreparedStatement ps_approv = con
							.prepareStatement("insert into crc_tbl_approval(CRC_No,U_Id,Approval_Id)values(?,?,?)");

					ps_approv.setInt(1, cr_no);
					ps_approv.setInt(2,
							Integer.parseInt(newuidapp.get(a).toString()));
					ps_approv.setInt(3, Approval_id);

					approv = ps_approv.executeUpdate();

					PreparedStatement ps_approv_hist = con
							.prepareStatement("insert into crc_tbl_approval_hist(CRC_Approval_Date_Hist,U_Id,CRC_No,Approval_Id)values(?,?,?,?)");
				...
				for (int s = 0; s < id1.size(); s++) {
					if (rsAppUid.getInt("U_Id") == Integer.parseInt(id1.get(s)
						...
						PreparedStatement ps_approve = con
								.prepareStatement("update crc_tbl_approval set Approval_Id=?,Remark=?,CRC_Approval_Date=? where crc_no="
										+ cr_no
										+ " and U_Id="
						...
						while (rs_date.next()) {
							original_date = rs_date
									.getTimestamp("CRC_Approval_Date");
						...
						PreparedStatement ps_approve_hist = con
								.prepareStatement("insert into crc_tbl_approval_hist(CRC_Approval_Date_Hist,U_Id,CRC_No,Approval_Id,Remark,CR_Status_Id,CRC_Approval_Date)values(?,?,?,?,?,?,?)");
						...
						while (rs_crtbl.next()) {
							...
							ascast = rs_crtbl
									.getDouble("Existing_As_Cast_Stock");

							existchlevel = rs_crtbl
									.getString("Existing_Change_level");
							newchangelevel = rs_crtbl
									.getString("New_Change_level");

							PreparedStatement ps_cust = con
									.prepareStatement("select * from customer_tbl where Cust_Id="
											+ rs_crtbl.getString("Cust_Id"));
						...
						PreparedStatement ps_dept = con
								.prepareStatement("select * from crc_tbl_cocern_dept where CRC_No="
										+ cr_no);
						ResultSet rs_dept = ps_dept.executeQuery();
						while (rs_dept.next()) {
							PreparedStatement ps_deptcon = con
									.prepareStatement("select * from user_tbl_dept where dept_id="
											+ rs_dept.getInt("dept_id"));
						...
						for (int at = 0; at < app_uid.size(); at++) {
							ps_auto = con
									.prepareStatement("select * from user_tbl where U_Id="
											+ Integer.parseInt(app_uid.get(at)
													.toString()));
