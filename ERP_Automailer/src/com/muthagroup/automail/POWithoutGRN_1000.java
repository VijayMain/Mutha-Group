...
public class POWithoutGRN_1000 extends TimerTask {
	...
	public void run() {
		...
		/*	if (!weekday[d.getDay()].equals("Tuesday") && d.getHours() == 15 && d.getMinutes() == 5){*/
				...
				String subject = "PO Without GRN List for amount greater than 1000 !!!";
				...
				PreparedStatement ps_rec = con.prepareStatement("select * from pending_approvee where type='cc' and report='PO_WithoutGRN_1000'");
				ResultSet rs_rec = ps_rec.executeQuery();
				while (rs_rec.next()) {
					cc_emails.add(rs_rec.getString("email"));
				}

				ps_rec = con.prepareStatement("select * from pending_approvee where type='to' and report='PO_WithoutGRN_1000'");
				rs_rec = ps_rec.executeQuery();
				while (rs_rec.next()) {
					to_emails.add(rs_rec.getString("email"));
				}

				ps_rec = con.prepareStatement("select * from pending_approvee where type='bcc' and report='PO_WithoutGRN_1000'");
				rs_rec = ps_rec.executeQuery();
				while (rs_rec.next()) {
				...
				sb.append("<b style='color: #0D265E;'>This is an automatically generated email for ERP - PO Without GRN for amount greater than 1000 dated " + simple_Date.format(date_last) + " !!!</b>");
				...
				/*
				 * exec "K1ERP"."dbo"."Sel_RptGRNRegisterMutha";1 '106', '0',
				 * '2026', '20160401', '20160820',
				 * '101,103,104,105,106,107,108,109,110,111,112,113,114,116,131'
				 * 
				 * MEPL H21 Start ==================== >
				 */
				...
				while (rs_getapp.next()) {
					if(Double.parseDouble(rs_getapp.getString("AMOUNT")) >= 1000.00){
					if (testavail == 0) {
					...
					sb.append("<tr style='font-size: 12px;border-width: 1px; padding: 8px; border-style: solid; border-color: #729ea5;'>"
							+ "<td align='right'>"
							+ rs_getapp.getString("GRN_NO")
							+ "</td>"
							+ "<td align='left'>"
							+ rs_getapp.getString("PRN_TRANDATE")
							+ "</td>"
							+ "<td align='right'>"
							+ rs_getapp.getString("INWNO")
							+ "</td>"
							+ "<td align='right'>"
							+ rs_getapp.getString("CHLNNO")
							+ "</td>"
							+ "<td align='left'>"
							+ rs_getapp.getString("CHLNDATE")
							+ "</td>"
							+ "<td align='left'>"
							+ rs_getapp.getString("MATNAME")
							+ "</td>"
							+ "<td align='left'>"
							+ rs_getapp.getString("AC_NAME")
							+ "</td>"
							+ "<td align='right'>"
							+ rs_getapp.getString("RECD_QTY")
							+ "</td>"
							+ "<td align='right'>"
							+ rs_getapp.getString("RATE")
							+ "</td>"
							+ "<td align='right'>"
							+ rs_getapp.getString("AMOUNT") + "</td>" + "</tr>");
