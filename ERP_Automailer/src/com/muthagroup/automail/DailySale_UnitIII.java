...
public class DailySale_UnitIII extends TimerTask { 
	@Override
	public void run() {
		try {
			//-------------------------------------------------------- Date Logic ---------------------------------------------------------
			...
			if (!weekday[d.getDay()].equals("Tuesday") && d.getHours() == 10 && d.getMinutes() == 13) { 
			/*	if (!weekday[d.getDay()].equals("Tuesday") && d.getHours() == 16 && d.getMinutes() == 41){*/
				...
				double lmsdom = 0,dpdom = 0,tsdom = 0,tmsdom = 0,additional=0,srdom = 0,nmsdom = 0;
				...
				String subject = "Daily Sale Report of MEPL UNIT III !!!"; 
				boolean sessionDebug = false;
				// *********************************************************************************************
				// multiple recipients : == >
				// ********************************************************************************************* 
				String recipients[] = {"azmutha@muthagroup.com","meghana@muthagroup.com","kunalvm@muthagroup.com","adgadkari@muthagroup.com","internalaudit@muthagroup.com","vmjoshi@muthagroup.com","ankatariya@muthagroup.com","jbaphna@muthagroup.com","nrfirodia@muthagroup.com"};
				...
				sb.append("<b style='color: #0D265E;font-family: Arial;font-size: 11px;'>*** This is an automatically generated email of Daily Sale Report !!! ***</b> <br />"+
						"<div style='width: 65%;float: left;'><b style='font-family: Arial;font-size: 14px;'>MUTHA ENGINEERING UNIT III Daily Sale Report as on "+ason_date +"</b></div>"+
				...
				while(rs.next()){
					if(rs.getString("CUSTOMER_NAME").equalsIgnoreCase("CASH IN HAND") || rs.getString("CUSTOMER_NAME").equalsIgnoreCase("STORE CASH IN HAND")){
						...
						lmsdom = lmsdom + Double.parseDouble(rs.getString("LMTH_NET_SLAMT"));
						dpdom = dpdom + Double.parseDouble(rs.getString("DSPT_SCH_SLAMT"));
						tsdom = tsdom + Double.parseDouble(rs.getString("TODAY_SLAMT"));
						tmsdom = tmsdom + Double.parseDouble(rs.getString("CURR_MTH_SLAMT"));
						additional = additional + sumach;
						srdom = srdom + Double.parseDouble(rs.getString("CM_SLRET_AMT"));
						nmsdom = nmsdom + Double.parseDouble(rs.getString("CM_NET_SLAMT"));  
					 }				
				}	
				rs.close();
				...
				sb.append("<tr style='font-size: 11px;background-color: #FCE6F2;'>"+
						...
						"<td style='text-align: right;'><b>"+twoDForm.format(lmsdom) +"</b> </td>"+
						"<td style='text-align: right;'><b>"+twoDForm.format(dpdom) +"</b> </td>"+
						"<td style='text-align: right;'><b>"+twoDForm.format(tsdom) +"</b> </td>"+
						"<td style='text-align: right;'><b>"+twoDForm.format(tmsdom) +"</b> </td>"+
						"<td style='text-align: right;'><b>"+ZeroDForm.format(additional) +"</b></td>"+
						"<td style='text-align: right;'><b>"+twoDForm.format(srdom) +"</b> </td>"+
						"<td style='text-align: right;'><b>"+twoDForm.format(nmsdom) +"</b> </td></tr>");				
				
				lmsdom = 0;dpdom = 0;tsdom = 0;tmsdom = 0;additional=0;srdom = 0;nmsdom = 0;
				
				sb.append("<tr style='font-size: 11px;'><td style='text-align: left;' colspan='8'><strong style='color: blue;'>DOMESTIC &#8658;</strong></td></tr>");
				
				ResultSet rsDom = cs.executeQuery();
				while (rsDom.next()) {				
					if(rsDom.getString("CAT_CODE").equalsIgnoreCase("1010001")){
						ach=0;
						lms1.add(Double.parseDouble(twoDForm.format(Double.parseDouble(rsDom.getString("LMTH_NET_SLAMT")))));
						dp1.add(Double.parseDouble(twoDForm.format(Double.parseDouble(rsDom.getString("DSPT_SCH_SLAMT")))));
						ts1.add(Double.parseDouble(twoDForm.format(Double.parseDouble(rsDom.getString("TODAY_SLAMT")))));
						tms1.add(Double.parseDouble(twoDForm.format(Double.parseDouble(rsDom.getString("CURR_MTH_SLAMT")))));
						sr1.add(Double.parseDouble(twoDForm.format(Double.parseDouble(rsDom.getString("CM_SLRET_AMT")))));
						nms1.add(Double.parseDouble(twoDForm.format(Double.parseDouble(rsDom.getString("CM_NET_SLAMT")))));
						if(!twoDForm.format(Double.parseDouble(rsDom.getString("DSPT_SCH_SLAMT"))).equalsIgnoreCase("0")){  
							ach = Double.parseDouble(twoDForm.format(Double.parseDouble(rsDom.getString("CM_NET_SLAMT"))))*100/Double.parseDouble(twoDForm.format(Double.parseDouble(rsDom.getString("DSPT_SCH_SLAMT"))));
							ach1.add(ach);
							} 
						sb.append("<tr style='font-size: 11px;'>"+
					"<td style='text-align: left;'>"+rsDom.getString("CUSTOMER_NAME") +"</td>"+
					"<td style='text-align: right;'>"+twoDForm.format(Double.parseDouble(rsDom.getString("LMTH_NET_SLAMT"))) +"</td>"+
			"<td style='text-align: right;'>"+twoDForm.format(Double.parseDouble(rsDom.getString("DSPT_SCH_SLAMT"))) +"</td>"+
			"<td style='text-align: right;'>"+twoDForm.format(Double.parseDouble(rsDom.getString("TODAY_SLAMT"))) +"</td>"+
			"<td style='text-align: right;'>"+twoDForm.format(Double.parseDouble(rsDom.getString("CURR_MTH_SLAMT"))) +"</td>"+
			"<td style='text-align: right;'>"+ZeroDForm.format(ach) +"</td>"+
			"<td style='text-align: right;'>"+twoDForm.format(Double.parseDouble(rsDom.getString("CM_SLRET_AMT"))) +"</td>"+
			"<td style='text-align: right;'>"+twoDForm.format(Double.parseDouble(rsDom.getString("CM_NET_SLAMT"))) +"</td></tr>"); 
					} 
				}
	...
	sumgrp_sumnms2 = sumgrp_sumnms2+nms;
	...
	sb.append("<tr style='font-size: 11px;background-color: #DEDEDE;'>"+
			...
			"<td style='text-align: right;'><b>"+twoDForm.format(sumgrp_sumnms2)+"</b></td></tr></table>");
	...
	System.out.println("ERP Mailer 13");	
	}
}
