...
public class Pending_Challan57f4_h25 extends TimerTask {

	@Override
	public void run() {
		try{
			System.out.println("ERP 57f4 Pending Approval H25 !!!");
			Date d = new Date(); 
			Date datesq = new Date();
			int day = datesq.getDate(); 
			
			if (day==1 && d.getHours() == 8 && d.getMinutes() == 12) {
				...
				/*System.out.println("Date H25 = " + nowDate);*/
			...
			String subject = "57F4 challans pending for over 30 days of MEPL H25";
			boolean sessionDebug = false;
			// *********************************************************************************************
			// multiple recipients : == >
			// *********************************************************************************************
			ArrayList to_emails = new ArrayList();
			PreparedStatement ps_rec = conLocal.prepareStatement("select * from pending_approvee where type='to' and report='Pending_Challan57f4_H25'");
			...
			sb.append("<b style='color: #0D265E;'>This is an automatically generated email for ERP - Pending Challan 57f4 List of MEPL H25</b>");
			...
			"<th>Rate</th>"+
			"</tr>");
			/*____________________________________________ MEPL H25 __________________________________________*/
 
			String comp = "102";
			Connection con_25 = ConnectionUrl.getMEPLH25ERP();
			ArrayList codes = new ArrayList();
			 PreparedStatement ps=con_25.prepareStatement("select * from MSTACCTGLSUB where SUB_GLCODE='12'");
			...
			*/
			CallableStatement cs = con_25.prepareCall("{call Sel_RptPendingChlnState(?,?,?,?)}");
			cs.setString(1, comp);
