...
public class Purchase_PendDues_StatK1 extends TimerTask {
	...
	public void run() {
		...
		if (!weekday[d.getDay()].equals("Tuesday") && d.getHours() == 15 && d.getMinutes() == 42) {
		...
		try {
			...
			ResultSet rs_category=null;
			 ...
			 PreparedStatement ps=con.prepareStatement("select * from K1ERP..MSTACCTGLSUB where SUB_GLCODE='12'");
		    ...
		    while(rs.next()){
		    	if(!rs.getString("TRAN_TYPE").equalsIgnoreCase("103")){
				    ...
				    sr++;
					    ...
					    ps_category = con.prepareStatement("select * from K1ERP..MSTMATCATAG  where CODE='"+rs.getString("CATEGOARY")+"'");
		...
		// multiple recipients : == >
		// ********************************************************************************************* 
		String recipients[] = {"msiplaccounts@muthagroup.com","manoj@muthagroup.com"};
		String cc_recipients[] = {"nileshss@muthagroup.com","internalaudit@muthagroup.com","kunalvm@muthagroup.com","adgadkari@muthagroup.com","ankatariya@muthagroup.com","vishal@muthagroup.com","govindmb@muthagroup.com"};
		...
		sb.append("<p style='font-size: 12px;'>*** This is an automatically generated alert email for Over due vendor payment of MEPL UNIT III !!! ***</p>");
		sb.append("<p style='font-size: 13px;'>Please find below attachment for Overdue Vendor Payment of MEPL UNIT III.</p>");
		...
		// ******************************************************************************
		transport.close(); 
		System.out.println("K1 Loop End");		
		}
			con.close();
	} catch (Exception e) {
	 e.printStackTrace();
		...
		System.out.println("Purchase Due 19 !!!");
	}
}