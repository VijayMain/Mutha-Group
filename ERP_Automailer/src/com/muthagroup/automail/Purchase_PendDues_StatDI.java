...
public class Purchase_PendDues_StatDI extends TimerTask {
	...
	public void run() {
		...
		if (!weekday[d.getDay()].equals("Tuesday") && d.getHours() == 15 && d.getMinutes() == 34) {
		...
		try {
			...
			Connection con = ConnectionUrl.getDIERPConnection(); 
			 PreparedStatement ps=con.prepareStatement("select * from DIERP..MSTACCTGLSUB where SUB_GLCODE='12'");
		    ...
		    while(rs.next()){
		    	if(!rs.getString("TRAN_TYPE").equalsIgnoreCase("103")){
				    ...
				    sr++;
					    ...
					    col++;
					    ps_category = con.prepareStatement("select * from DIERP..MSTMATCATAG  where CODE='"+rs.getString("CATEGOARY")+"'");
		...
		// multiple recipients : == >
		// ********************************************************************************************* 
		String recipients[] = {"diaccounts@muthagroup.com"};
		String cc_recipients[] = {"srpatekar@muthagroup.com","nileshss@muthagroup.com","internalaudit@muthagroup.com","kunalvm@muthagroup.com","adgadkari@muthagroup.com","ankatariya@muthagroup.com","kamlesh@muthagroup.com","govindmb@muthagroup.com"};
		...
		sb.append("<p style='font-size: 12px;'>*** This is an automatically generated alert email for Over due vendor payment of DHANASHREE INDUSTRIES !!! ***</p>");
		 
		sb.append("<p style='font-size: 13px;'>Please find below attachment for Overdue Vendor Payment of DHANASHREE INDUSTRIES.</p>");
		...
		// ******************************************************************************
		transport.close(); 
		System.out.println("MEPL H25 Loop End");		
		}
			con.close();
	} catch (Exception e) {
	 e.printStackTrace();
		...
		System.out.println("Purchase Due 20 !!!");
	}
}
